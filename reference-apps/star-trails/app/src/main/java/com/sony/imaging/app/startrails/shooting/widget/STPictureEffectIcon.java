...
public class STPictureEffectIcon extends ActiveImage {
    ...
    public STPictureEffectIcon(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected NotificationListener getNotificationListener() {
        if (this.mListener == null) {
            this.mListener = new ActiveImage.ActiveImageListener() { // from class: com.sony.imaging.app.startrails.shooting.widget.STPictureEffectIcon.1
                ...
                public void onNotify(String tag) {
                    super.onNotify(tag);
                    STPictureEffectIcon.this.refresh();
                }
    ...
    protected String getValue() {
        String value;
        String mode = BackUpUtil.getInstance().getPreferenceString(STBackUpKey.CUSTOM_PICTURE_EFFECT_KEY, "off");
        if (STUtility.getInstance().isSilenShutterOFF() && STUtility.getInstance().getCurrentTrail() == 2 && (PictureEffectController.MODE_MINIATURE.equals(mode) || PictureEffectController.MODE_SOFT_FOCUS.equals(mode))) {
            value = BackUpUtil.getInstance().getPreferenceString(STBackUpKey.CUSTOM_PICTURE_EFFECT_KEY_OPTION, "off");
        } else {
            PictureEffectController controller = PictureEffectController.getInstance();
            value = controller.getValue();
            mode = controller.getValue(PictureEffectController.PICTUREEFFECT);
        }
