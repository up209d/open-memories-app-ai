...
public class FolderBar extends ImageView {
    ...
    private int getTouchedArea(float xPos, float yPos) {
        ...
        float xEnd = STConstants.INVALID_APERTURE_VALUE + btnWidth;
        ...
        if (xPos >= STConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= STConstants.INVALID_APERTURE_VALUE && yPos < yUpperEnd) {
            return 0;
        }
        if (xPos >= STConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= yCenterStart && yPos < yCenterEnd) {
            return 1;
        }
        if (xPos < STConstants.INVALID_APERTURE_VALUE || xPos >= xEnd || yPos < yLowerStart || yPos >= btnHeight) {
            return -1;
        }
        return 2;
    }
