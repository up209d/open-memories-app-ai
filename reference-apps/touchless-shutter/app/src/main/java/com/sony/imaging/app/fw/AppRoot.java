...
public abstract class AppRoot extends Activity {
    ...
    private IKeyDispatch mKeyDispatcher;
    View mView;
                        ...
                        if (view != null) {
                            if (executed.contains(objectLayout)) {
                                ...
                                if (objectIndex != targetIndex) {
                                    ...
                                    AppRoot.log_string.replace(0, AppRoot.log_string.length(), LOG_VIEW_CHANGED_PREFIX).append(objectLayout.getClass().getSimpleName()).append(LOG_VIEW_CHANGED_FROM).append(objectIndex).append(LOG_VIEW_CHANGED_TO).append(targetIndex);
                                    Log.w(TAG, AppRoot.log_string.toString());
                        ...
                        switch (op.op) {
                            case 0:
                                if (!container.handle.isAlive()) {
                                    AppRoot.log_string.replace(0, AppRoot.log_string.length(), log_container_prefix).append(container.getClass().getSimpleName()).append(log_container_error).append(target.getClass().getSimpleName()).append(log_container_suffix);
                                    Log.e(TAG, AppRoot.log_string.toString());
                                    break;
                                } else {
                            ...
                            case 1:
                                if (!op.handle.isAlive()) {
                                    AppRoot.log_string.replace(0, AppRoot.log_string.length(), log_prefix).append(op.handle.state.getClass().getSimpleName()).append(log_suffix).append(container.getClass().getSimpleName());
                                    Log.e(TAG, AppRoot.log_string.toString());
                                    break;
                                } else {
                            ...
                            case 2:
                                if (!container.handle.isAlive() || !op.handle.isAlive()) {
                                    AppRoot.log_string.replace(0, AppRoot.log_string.length(), log_prefix).append(target.handle.state.getClass().getSimpleName()).append(log_error).append(target.getClass().getSimpleName()).append(log_suffix).append(container.getClass().getSimpleName());
                                    Log.e(TAG, AppRoot.log_string.toString());
                                    break;
                                } else {
                                    target.container = container;
                                    ...
                                    target.depth = container.getDepth() + 1;
                                    target.depthOffset = container.depthOffset / 2;
                                    container.currentState.get(op.handle.layer).state.remove();
    ...
    /* loaded from: classes.dex */
    private class SystemReady implements ScalarSystemManager.SystemReadyEventListener {
    ...
    public static final class USER_KEYCODE {
        ...
        public static final int RESET = 65286;
        ...
        public static final int WIFI = 65285;
        ...
        private static void initKeys() {
            if (sKeys == null) {
                sKeys = new ArrayList<>(KEY_CAPACITY);
                sKeys.add(Integer.valueOf(UP));
                ...
                sKeys.add(Integer.valueOf(SHOOTING_MODE));
