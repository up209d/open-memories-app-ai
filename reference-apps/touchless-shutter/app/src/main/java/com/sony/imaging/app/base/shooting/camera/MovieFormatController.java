        ...
        public void onNotify(String tag) {
            if (CameraNotificationManager.SCENE_MODE.equals(tag)) {
                ...
                if (!recSetting.equals(current)) {
                    MovieFormatController.sBuilder.replace(0, MovieFormatController.sBuilder.length(), MovieFormatController.MSG_SET_INHIBITED_SETTING_BACK).append(recSetting);
                    Log.w(MovieFormatController.TAG, MovieFormatController.sBuilder.toString());
    ...
    protected boolean setAvailableSetting() {
        ...
        if (movieRecordSetting != null) {
            ...
            if (this.mUserSetting.get(key) == null) {
                ...
                sBuilder.replace(0, sBuilder.length(), MSG_REMEMBER_UNAVAILABLE_SETTING).append(movieRecordSetting);
                Log.i(TAG, sBuilder.toString());
