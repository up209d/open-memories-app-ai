...
public class FolderBar extends ImageView {
    ...
    private int getTouchedArea(float xPos, float yPos) {
        ...
        if (xPos >= 0.0f && xPos < xEnd && yPos >= 0.0f && yPos < yUpperEnd) {
            return 0;
        }
        if (xPos >= 0.0f && xPos < xEnd && yPos >= yCenterStart && yPos < yCenterEnd) {
            return 1;
        }
        if (xPos < 0.0f || xPos >= xEnd || yPos < yLowerStart || yPos >= btnHeight) {
            return -1;
        }
        return 2;
    }
