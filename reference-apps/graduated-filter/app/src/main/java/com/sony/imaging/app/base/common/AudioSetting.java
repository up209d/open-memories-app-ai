...
public class AudioSetting {
    ...
    static {
        ...
        if (AudioRecordingController.isSupportedSettingChangedListener()) {
            AUDIO_SETTING_CHANGE_NOTIFY_TAGS.append(2, AudioNotificationManager.AUDIO_RECORDING_CHANGED);
        }
    ...
    public void setAudioParameters(AudioManager.Parameters params) {
        ...
        if (!ExecutorCreator.getInstance().isCameraModeMovie() && !Environment.isAudioManagerAvailableInAnyMode()) {
        ...
        while (fromTokenizer.hasMoreElements()) {
            ...
            if (kv != null) {
                ...
                if (!this.isCameraOpened) {
                    ...
                    if (!ExecutorCreator.getInstance().isCameraModeMovie() && mMovieModeOnlyList.contains(kv.first)) {
                    }
                    to.set((String) kv.first, (String) kv.second);
                    isSet = true;
                }
    ...
    private void createParameterComparatorList() {
        ...
        if (AudioRecordingController.isUnSupportedSettingChangedListener()) {
            this.AUDIO_PARAMETER_COMPARATORS.add(new AudioSimpleComparator<Boolean>(AudioNotificationManager.AUDIO_RECORDING_CHANGED) { // from class: com.sony.imaging.app.base.common.AudioSetting.2
                /* JADX INFO: Access modifiers changed from: protected */
                @Override // com.sony.imaging.app.base.common.comparator.ParameterComparator._SimpleComparator
                public Boolean getValue(AudioManager.Parameters obj) {
                    return Boolean.valueOf(obj.getMicrophoneEnable());
                }
            });
        }
        if (Environment.isBeepAPISupported()) {
            this.AUDIO_PARAMETER_COMPARATORS.add(new AudioSimpleComparator<String>("BeepModeChanged") { // from class: com.sony.imaging.app.base.common.AudioSetting.3
                /* JADX INFO: Access modifiers changed from: protected */
                @Override // com.sony.imaging.app.base.common.comparator.ParameterComparator._SimpleComparator
                public String getValue(AudioManager.Parameters obj) {
                    return obj.getOutputBeep();
                }
            });
        }
    }
    ...
    public void onCameraOpened() {
        ...
        if (ExecutorCreator.getInstance().isCameraModeMovie()) {
            this.mSupportedAudioParameters = this.mAudioManager.getSupportedParameters();
            this.mAudioParameters = this.mAudioManager.getParameters();
            initializeAudioParameters();
            initAudioListeners();
