...
public class AutoReviewLayout extends Layout {
    private static final int API_VERSION_SUPPORTING_MKNOTE_METERING_SETTING = 16;
    ...
    protected void displayInfo(CameraEx.ReviewInfo info) {
        ...
        if (dispMode == 5) {
            ...
            if (16 <= Environment.getVersionPfAPI()) {
                if (photo.bMkNoteSettingOfMeteringModeValid) {
                    meteringMode.setValue(photo.mkNoteSettingOfMeteringMode, true);
                } else if (photo.bMeteringModeValid) {
                    meteringMode.setValue(photo.meteringMode, false);
                }
            } else if (photo.bMeteringModeValid) {
                meteringMode.setValue(photo.meteringMode, false);
            }
