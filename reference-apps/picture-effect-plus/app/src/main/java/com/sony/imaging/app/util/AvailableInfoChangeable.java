...
public class AvailableInfoChangeable extends AvailableInfoImpl {
    ...
    private static final StringBuilder SELCTOR = new StringBuilder();
    ...
    protected void initialize() {
        ...
        int count = -1;
        try {
            SELCTOR.replace(0, SELCTOR.length(), "select count(*) from factor_records").append(MovieFormatController.Settings.SEMI_COLON);
            if (AvailableInfo.DEBUG) {
                Log.i(TAG, "start query: " + ((Object) SELCTOR));
            }
            c = this.mDB.rawQuery(SELCTOR.toString(), null);
    ...
    protected String getDataBasePath() {
        return this.mContext.getFilesDir() + "/" + this.mDbPath;
    }
}
