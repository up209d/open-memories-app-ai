...
public class DisplayModeObserver extends NotificationManager {
    ...
    private void initializeActiveDisplayModeIndex() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_DISPMODES).append(LOG_MSG_LCD).append(dispModeLCD).append(", ").append(LOG_MSG_EVF).append(dispModeEVF).append(", ").append(LOG_MSG_PBS).append(dispModePbS).append(", ");
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_DISPMODEINDEXES).append(LOG_MSG_LCD).append(indexRecLCD2).append(", ").append(LOG_MSG_EVF).append(indexRecEVF2).append(", ").append(LOG_MSG_PBS).append(indexPbS).append(", ");
        Log.i(TAG, STRBUILD.toString());
    ...
    public void resume() {
        ...
        if (mYUVRect != null) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_ONLAYOUTCHANGED).append(LOG_MSG_RECT).append(LOG_MSG_ROUND_BRACKET_L).append(mYUVRect.pxLeft).append(", ").append(mYUVRect.pxTop).append(LOG_MSG_ROUND_BRACKET_R).append(" - ").append(LOG_MSG_ROUND_BRACKET_L).append(mYUVRect.pxRight).append(", ").append(mYUVRect.pxBottom).append(LOG_MSG_ROUND_BRACKET_R);
            Log.i(TAG, STRBUILD.toString());
        }
    }
    ...
    public void toggleDisplayMode(int appMode) {
        ...
        int dispModeIndex = (this.mActiveDisplayModeIndex[appMode][this.mActiveDevice] + 1) % dispModeCount;
        setDisplayModeIndex(appMode, skipDigitalLevel(appMode, this.mActiveDevice, dispModeCount, dispModeIndex));
    }
    ...
    public void setDisplayMode(int appMode, int dispMode) {
        ...
        if (index > -1) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_NO_EXIST_DISPLAYMODE).append(appMode).append(" - ").append(dispMode);
    ...
    public int obtainActiveDeviceId() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETACTIVEDEVICE).append(device);
        Log.i(TAG, STRBUILD.toString());
        int deviceId = retrieveDeviceId(device);
        if (-1 == deviceId) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_ILLEGALDEVICE).append(device);
            Log.w(TAG, STRBUILD.toString());
            return 0;
        }
        return deviceId;
    ...
    class OnDisplayEventListener implements DisplayManager.DisplayEventListener {
        ...
        public void onDeviceStatusChanged(int eventId) {
            DisplayModeObserver.STRBUILD.replace(0, DisplayModeObserver.STRBUILD.length(), DisplayModeObserver.LOG_MSG_CANNOTGETLENSINFO).append(DisplayModeObserver.LOG_MSG_EVENTID).append(eventId);
            Log.i(DisplayModeObserver.TAG, DisplayModeObserver.STRBUILD.toString());
            if (4096 == eventId) {
                int newDevice = DisplayModeObserver.this.obtainActiveDeviceId();
                if (DisplayModeObserver.this.mActiveDevice != newDevice) {
    ...
    /* loaded from: classes.dex */
    class OnVideoLayoutStatusListener implements DisplayManager.VideoLayoutStatusListener {
        OnVideoLayoutStatusListener() {
        }

        public void onLayoutChanged(DisplayManager.VideoRect rect) {
            DisplayModeObserver.STRBUILD.replace(0, DisplayModeObserver.STRBUILD.length(), DisplayModeObserver.LOG_MSG_ONLAYOUTCHANGED).append(DisplayModeObserver.LOG_MSG_RECT).append(DisplayModeObserver.LOG_MSG_ROUND_BRACKET_L).append(rect.pxLeft).append(", ").append(rect.pxTop).append(DisplayModeObserver.LOG_MSG_ROUND_BRACKET_R).append(" - ").append(DisplayModeObserver.LOG_MSG_ROUND_BRACKET_L).append(rect.pxRight).append(", ").append(rect.pxBottom).append(DisplayModeObserver.LOG_MSG_ROUND_BRACKET_R);
            Log.i(DisplayModeObserver.TAG, DisplayModeObserver.STRBUILD.toString());
    ...
    protected int skipDigitalLevel(int appMode, int activeDevice, int dispModeCount, int dispModeIndex) {
        if (3 <= CameraSetting.getPfApiVersion()) {
            ...
            Log.d(TAG, "skipDigitalLevel -> isSupporeted = " + isSupporeted);
            if (isSupporeted == 0) {
