...
public class StillFolderViewMode extends ViewMode {
    private static final String[] GROUP_QUERY_PROJECTION = {"_id", "_count", "count_of_one_before", "dcf_folder_number"};
    ...
    protected synchronized boolean queryInGroup() {
        boolean z;
        int dcfFolderNum = this.mGroupCursor.getInt(this.mGroupCursor.getColumnIndex("dcf_folder_number"));
    ...
    public synchronized int findContents(SearchInfo info) {
        ...
        if (folderNumber < 0 || fileNumber < 0) {
            ...
            Log.i(TAG, LogHelper.getScratchBuilder(MSG_SEARCH_CONTENTS).append(info.folderNumber).append(LogHelper.MSG_COMMA).append(info.fileNumber).append(LogHelper.MSG_COLON).append(result).toString());
        }
        return result;
    }
