...
public class NormalExecutor extends ShootingExecutor {
    ...
    public class NormalShutterCb extends ShootingExecutor.MyShutterCb {
        ...
        public void onShutter(int status, CameraEx cameraEx) {
            ...
            Log.d(NormalExecutor.TAG, NormalExecutor.LOG_CANCEL_TAKE_PICTURE);
    ...
    public void lockAutoFocus(boolean lock) {
        if (isFocusLocked != lock) {
            isFocusLocked = lock;
            if (!lock) {
                STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), "cancelAutoFocus").append(LOG_STATUS).append(sIsAutoFocusRequested);
                Log.d(TAG, STRBUILD_CAMERA.toString());
                if (!sIsAutoFocusRequested) {
                    STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), "cancelAutoFocus").append(LOG_STATUS).append("lock");
                    Log.d(TAG, STRBUILD_CAMERA.toString());
