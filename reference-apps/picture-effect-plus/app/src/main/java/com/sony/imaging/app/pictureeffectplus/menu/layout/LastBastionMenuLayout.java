...
public class LastBastionMenuLayout extends BaseMenuLayout {
    ...
    public void onPause() {
        AppLog.enter(TAG, AppLog.getMethodName());
        CautionUtilityClass.getInstance().disapperTrigger(PictureEffectPlusCaution.CAUTION_ID_DLAPP_CHANGE_DIAL_TO_PASM);
        CautionUtilityClass.getInstance().setDispatchKeyEvent(PictureEffectPlusCaution.CAUTION_ID_DLAPP_CHANGE_DIAL_TO_PASM, null);
        super.onPause();
        AppLog.exit(TAG, AppLog.getMethodName());
    }
    ...
    protected void displayCaution() {
        AppLog.enter(TAG, AppLog.getMethodName());
        IkeyDispatchEach mKey = new IkeyDispatchEach() { // from class: com.sony.imaging.app.pictureeffectplus.menu.layout.LastBastionMenuLayout.1
        ...
        CautionUtilityClass.getInstance().requestTrigger(PictureEffectPlusCaution.CAUTION_ID_DLAPP_CHANGE_DIAL_TO_PASM);
        CautionUtilityClass.getInstance().setDispatchKeyEvent(PictureEffectPlusCaution.CAUTION_ID_DLAPP_CHANGE_DIAL_TO_PASM, mKey);
        AppLog.exit(TAG, AppLog.getMethodName());
    }
}
