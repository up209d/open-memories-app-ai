...
import com.sony.imaging.app.base.shooting.CaptureStateBase;
...
public class S1OffEEStateKeyHandler extends ShootingKeyHandlerBase {
    ...
    public int pushedIRShutterKey() {
        ...
        State state = (State) this.target;
        Bundle data = new Bundle();
        data.putBoolean(CaptureStateBase.TAKE_PICTURE_BY_REMOTE, true);
    ...
    public int turnedEVDial() {
        boolean ret = ExposureCompensationController.getInstance().setEvDialValue(EVDialDetector.getEVDialPosition());
        if (ret) {
            ((IUserChanging) this.target).changeExposure();
            return 1;
        }
