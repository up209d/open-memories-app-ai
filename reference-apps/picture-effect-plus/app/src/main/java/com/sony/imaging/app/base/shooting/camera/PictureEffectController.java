...
public class PictureEffectController extends AbstractController {
    ...
    protected void createSupported() {
        ...
        if (this.mRawSupportedPictureEffects == null) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDPICTUREEFFECTS).append(this.mRawSupportedPictureEffects);
    ...
    private void setPictureEffect(Pair<Camera.Parameters, CameraEx.ParametersModifier> p, String value) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECT).append(value);
        Log.i(TAG, STRBUILD.toString());
    ...
    private void setOptionValue(Pair<Camera.Parameters, CameraEx.ParametersModifier> p, String itemId, String value) {
        if (MODE_TOY_CAMERA.equals(itemId)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECTPOSTERIZATIONEFFECT).append(value);
            Log.i(TAG, STRBUILD.toString());
        } else if (MODE_PART_COLOR.equals(itemId)) {
            ((CameraEx.ParametersModifier) p.second).setPictureEffectPartColorEffect(value);
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECTPARTCOLOREFFECT).append(value);
            Log.i(TAG, STRBUILD.toString());
            ...
            if (value.startsWith(SOFT_FOCUS_PREFIX)) {
                ...
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECTSOFTFOCUSEFFECTLEVEL).append(value);
                Log.i(TAG, STRBUILD.toString());
            }
            ...
            if (value.startsWith(HDR_ART_PREFIX)) {
                ...
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECTHDRARTEFFECTLEVEL).append(value);
                Log.i(TAG, STRBUILD.toString());
            }
        } else if (MODE_MINIATURE.equals(itemId)) {
            ((CameraEx.ParametersModifier) p.second).setPictureEffectMiniatureFocusArea(value);
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTUREEFFECTMINIATUREFOCUSAREA).append(value);
            Log.i(TAG, STRBUILD.toString());
        } else if ("illust".equals(itemId)) {
    ...
    public String getValue(String itemId) {
        ...
        if (PICTUREEFFECT.equals(itemId)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECT).append(ret);
            Log.i(TAG, STRBUILD.toString());
            return ret;
        }
    ...
    private String getOptionValue(Pair<Camera.Parameters, CameraEx.ParametersModifier> p, String itemId) {
        if (MODE_TOY_CAMERA.equals(itemId)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTTOYCAMERAEFFECT).append(ret);
            Log.i(TAG, STRBUILD.toString());
            return ret;
        }
        if (MODE_POSTERIZATION.equals(itemId)) {
            String ret2 = ((CameraEx.ParametersModifier) p.second).getPictureEffectPosterizationEffect();
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTPOSTERIZATIONEFFECT).append(ret2);
            Log.i(TAG, STRBUILD.toString());
            return ret2;
        }
        if (MODE_PART_COLOR.equals(itemId)) {
            String ret3 = ((CameraEx.ParametersModifier) p.second).getPictureEffectPartColorEffect();
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTPARTCOLOREFFECT).append(ret3);
            Log.i(TAG, STRBUILD.toString());
            return ret3;
        }
        if (MODE_SOFT_FOCUS.equals(itemId)) {
            int level = ((CameraEx.ParametersModifier) p.second).getPictureEffectSoftFocusEffectLevel();
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTSOFTFOCUSEFFECTLEVEL).append(level);
            Log.i(TAG, STRBUILD.toString());
        ...
        if (MODE_HDR_ART.equals(itemId)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTHDRARTEFFECTLEVEL).append(level2);
            Log.i(TAG, STRBUILD.toString());
        ...
        if (MODE_MINIATURE.equals(itemId)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTUREEFFECTMINIATUREFOCUSAREA).append(ret4);
            Log.i(TAG, STRBUILD.toString());
            return ret4;
        }
        if ("illust".equals(itemId)) {
            int level3 = ((CameraEx.ParametersModifier) p.second).getPictureEffectIllustEffectLevel();
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETMINPICTUREEFFECTILLUSTEFFECTLEVEL).append(level3);
            Log.i(TAG, STRBUILD.toString());
    ...
    public List<String> createSupportedOptionValueArray(Pair<Camera.Parameters, CameraEx.ParametersModifier> params, String tag) {
        ...
        if (MODE_POSTERIZATION.equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDPICTUREEFFECTPOSTERIZATIONEFFECTS).append(list2);
            Log.i(TAG, STRBUILD.toString());
            return list2;
        }
        if (MODE_PART_COLOR.equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDPICTUREEFFECTPARTCOLOREFFECTS).append(list3);
            Log.i(TAG, STRBUILD.toString());
            return list3;
        }
        if (MODE_SOFT_FOCUS.equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETMINPICTUREEFFECTSOFTFOCUSEFFECTLEVEL).append(this.mSoftfocusMin).append(", ").append(LOG_MSG_GETMAXPICTUREEFFECTSOFTFOCUSEFFECTLEVEL).append(this.mSoftfocusMax);
            Log.i(TAG, STRBUILD.toString());
        ...
        if (MODE_HDR_ART.equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETMINPICTUREEFFECTHDRARTEFFECTLEVEL).append(this.mHdrMin).append(", ").append(LOG_MSG_GETMAXPICTUREEFFECTHDRARTEFFECTLEVEL).append(this.mHdrMax);
            Log.i(TAG, STRBUILD.toString());
        ...
        if (MODE_MINIATURE.equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDPICTUREEFFECTMINIATUREFOCUSAREAS).append(list6);
            Log.i(TAG, STRBUILD.toString());
            return list6;
        }
        if (2 <= Environment.getVersionOfHW() && "illust".equals(tag)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETMINPICTUREEFFECTILLUSTEFFECTLEVEL).append(this.mIllustMin).append(", ").append(LOG_MSG_GETMAXPICTUREEFFECTILLUSTEFFECTLEVEL).append(this.mIllustMax);
            Log.i(TAG, STRBUILD.toString());
        ...
        for (String mode : this.mRawSupportedPictureEffects) {
            if (this.ITEM_ID_NEEDS_OPTION.contains(mode)) {
                ...
                if (options != null && !options.isEmpty()) {
                    if (Environment.getVersionOfHW() < 2 && mode.equals("illust")) {
                        break;
                    }
                    list.add(mode);
                }
