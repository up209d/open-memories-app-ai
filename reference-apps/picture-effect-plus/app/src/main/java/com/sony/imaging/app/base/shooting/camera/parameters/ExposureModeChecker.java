...
public class ExposureModeChecker extends StringValuesChecker {
    ...
    public void check(String value, Camera.Parameters supported, Camera.Parameters to, Camera.Parameters from) {
        if (value != null) {
            ...
            boolean inh = 2 == mode && isPhaseDetectAF();
            while (true) {
                if (!tokenizer.hasMoreElements()) {
                    break;
                }
                String elem = tokenizer.nextToken();
                if (true != inh || (!"aperture-priority".equals(elem) && !"shutter-speed".equals(elem) && !"manual-exposure".equals(elem))) {
            ...
            if (!isSupported) {
                if ("picture-effect".equals(value)) {
                    if (isPAutoSupported) {
