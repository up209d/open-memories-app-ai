...
public class PictureSizeController extends ShootingModeController {
    ...
    public void setValue(String itemId, String value) {
        if (MENU_ITEM_ID_ASPECT_RATIO.equals(itemId)) {
            if (1 <= CameraSetting.getPfApiVersion()) {
                ...
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETIMAGEASPECT).append(value);
        ...
        if (1 <= CameraSetting.getPfApiVersion()) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETIMAGESIZE).append(value);
            Log.i(TAG, STRBUILD.toString());
    ...
    private void setPictureSize(String aspect, String size) {
        ...
        if (list == null) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_ASPECT).append(aspect);
            Log.e(TAG, STRBUILD.toString());
            return;
        }
        int index = SIZES.indexOf(size);
        if (-1 == index) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_SIZE).append(size);
            Log.e(TAG, STRBUILD.toString());
            return;
        }
        Pair<Camera.Parameters, CameraEx.ParametersModifier> params = this.mCameraSetting.getEmptyParameters();
        ((Camera.Parameters) params.first).setPictureSize(list.get(index).width, list.get(index).height);
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETPICTURESIZE).append(list.get(index).width).append(", ").append(list.get(index).height);
        Log.i(TAG, STRBUILD.toString());
        this.mCameraSetting.setParameters(params);
    }
    ...
    public String getValue(String itemId) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_ITEMID).append(itemId);
        Log.e(TAG, STRBUILD.toString());
        return null;
    }
    ...
    private String getSize() {
        ...
        if (1 <= CameraSetting.getPfApiVersion()) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETIMAGESIZE).append(pfValue);
            Log.i(TAG, STRBUILD.toString());
            result = convertToApp(pfValue);
            if (result == null) {
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_SIZE).append(pfValue);
                Log.e(TAG, STRBUILD.toString());
            }
        } else {
            Camera.Size size = ((Camera.Parameters) params.first).getPictureSize();
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETPICTURESIZE).append(size.width).append(", ").append(size.height);
            Log.i(TAG, STRBUILD.toString());
            ArrayList<Dimension> list = this.mSizeData.getSizeList(getAspect());
            if (list == null) {
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_ASPECT).append(getAspect());
                Log.e(TAG, STRBUILD.toString());
                return null;
            }
            int num = list.size();
    ...
    private String getAspect() {
        ...
        if (1 <= CameraSetting.getPfApiVersion()) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETIMAGEASPECT).append(pfValue);
            Log.i(TAG, STRBUILD.toString());
            String result = convertToApp(pfValue);
            if (result == null) {
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_ASPECT).append(pfValue);
                Log.e(TAG, STRBUILD.toString());
                return result;
            }
            return result;
    ...
    private static List<String> createSupportedValueArray(PictureSizeData sizeData, String aspect, String tag) {
        ...
        if (MENU_ITEM_ID_IMAGE_SIZE.equals(tag)) {
            ...
            if (list == null) {
                ...
                STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_SIZE).append(list.size());
                Log.e(TAG, STRBUILD.toString());
            } else {
    ...
    public static class PictureSizeData {
        ...
        public PictureSizeData() {
            ...
            if (sizelist != null) {
                PictureSizeController.STRBUILD.replace(0, PictureSizeController.STRBUILD.length(), PictureSizeController.LOG_MSG_GETSUPPORTEDPICTURESIZES);
                for (Camera.Size size : sizelist) {
                    PictureSizeController.STRBUILD.append(PictureSizeController.LOG_MSG_SQUARE_BRACKET_L).append(size.width).append(", ").append(size.height).append(PictureSizeController.LOG_MSG_SQUARE_BRACKET_R);
                }
                Log.i(PictureSizeController.TAG, PictureSizeController.STRBUILD.toString());
                if (!sizelist.isEmpty()) {
            ...
            PictureSizeController.STRBUILD.replace(0, PictureSizeController.STRBUILD.length(), PictureSizeController.LOG_MSG_GETSUPPORTEDPICTURESIZES).append(PictureSizeController.LOG_MSG_NULL);
            Log.i(PictureSizeController.TAG, PictureSizeController.STRBUILD.toString());
        }
