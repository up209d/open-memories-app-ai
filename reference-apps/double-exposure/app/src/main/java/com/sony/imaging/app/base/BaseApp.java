...
public abstract class BaseApp extends AppRoot {
    ...
    private static final String VERSION = "1.114";
    ...
    protected static int toExecutorCreatorRecMode(int mode) {
        if (3 == mode) {
            return 3;
        }
        if (1 == mode) {
    ...
    public final AppRoot.OpenStateData _getStartApp() {
        if (!isHousingSupported()) {
            ...
            Log.i(TAG, "Housing Status " + status.valid + LogHelper.MSG_COMMA + status.status);
    ...
    public BaseApp() {
        Log.i(TAG, "BaseApp version 1.114");
    ...
    public void setBootLogo(int iconId, int stringId) {
        setBootLogo(iconId, stringId, Integer.MIN_VALUE);
    }
    ...
    public void onBoot(BootFactor factor) {
        ...
        DisplayModeObserver.getInstance().resume();
        ...
        creator.setSupportingRecMode(toExecutorCreatorRecMode(getSupportingRecMode()));
        ...
        FocusModeController.getInstance().keepToggleFocusModeFlag(false);
    }
