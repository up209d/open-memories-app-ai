...
public class DeleteMultipleIndexLayout extends SelectorIndexLayout {
    ...
    protected int getLayoutResource() {
        ...
        if (2 == aspect) {
            retVal = R.layout.pb_layout_deletor_multiple_selecting_indexpb_4x3;
        } else {
            retVal = R.layout.pb_layout_deletor_multiple_selecting_indexpb_3x3;
        }
        AppLog.exit(this.TAG, AppLog.getMethodName());
        return retVal;
    }
    ...
    public boolean onGroupEntered(int[] result) {
        AppLog.enter(this.TAG, AppLog.getMethodName());
        boolean bRetVal = super.onGroupEntered(result);
        if (result[0] == -2) {
            CautionUtilityClass.getInstance().requestTrigger(329);
        }
        AppLog.exit(this.TAG, AppLog.getMethodName());
