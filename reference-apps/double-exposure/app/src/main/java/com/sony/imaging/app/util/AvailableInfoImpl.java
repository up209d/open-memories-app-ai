...
public abstract class AvailableInfoImpl {
    ...
    private final StringBuilder SELCTOR;
    ...
    public AvailableInfoImpl() {
        this.mDB = null;
        this.SELCTOR = new StringBuilder();
        this.mDbPath = null;
    }
    ...
    */
    ...
    private Cursor getFactorCursor(List<String> inhFeatureIDs) {
        ...
        if (!inhFeatureIDs.isEmpty()) {
            this.SELCTOR.replace(0, this.SELCTOR.length(), GET_FACTOR);
            for (String feature : inhFeatureIDs) {
                this.SELCTOR.append(feature).append(END_SIGN).append(SEPARATER);
            }
            int lastIndex = this.SELCTOR.length() - 1;
            this.SELCTOR.delete(lastIndex - SEPARATER.length(), lastIndex);
            this.SELCTOR.append(")");
            if (AvailableInfo.DEBUG) {
                Log.i(TAG, "start query: " + ((Object) this.SELCTOR));
            }
            cursor = this.mDB.rawQuery(this.SELCTOR.toString(), null);
            if (AvailableInfo.DEBUG) {
                Log.i(TAG, "finish");
            }
        }
    ...
    */
    ...
    public int isFactorHelper(String factorID) {
        int ret = -1;
        this.SELCTOR.replace(0, this.SELCTOR.length(), GET_FACTOR_BIT).append(factorID).append(END_SIGN);
        if (AvailableInfo.DEBUG) {
            Log.i(TAG, "start query: " + ((Object) this.SELCTOR));
        }
        Cursor c = null;
        try {
            try {
                c = this.mDB.rawQuery(this.SELCTOR.toString(), null);
                if (AvailableInfo.DEBUG) {
                    Log.i(TAG, "finish");
                }
    ...
    protected static String convert2Hex(byte[] factor) {
        ...
        if (factor != null) {
            for (byte b : factor) {
                data = data + String.format("%02x", Integer.valueOf(b & 255));
            }
        }
        return data;
    }
