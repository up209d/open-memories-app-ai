...
public class SecondShootingIcon extends ActiveImage {
    private final String TAG;

    public SecondShootingIcon(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void refresh() {
        ...
        if (DoubleExposureUtil.getInstance().getCurrentShootingScreen().equalsIgnoreCase(DoubleExposureConstant.FIRST_SHOOTING)) {
            if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.SKY) || selectedTheme.equalsIgnoreCase(ThemeSelectionController.SIL) || selectedTheme.equalsIgnoreCase(ThemeSelectionController.TEXTURE)) {
                drawable = R.drawable.p_16_dd_parts_mle_any_disable;
            } else if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.SOFTFILTER)) {
                drawable = R.drawable.p_16_dd_parts_mle_softfilter_2nd_disable;
            } else if (selectedTheme.equalsIgnoreCase("Rotation")) {
                drawable = R.drawable.p_16_dd_parts_mle_rotation_2nd_disable;
            } else if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.MIRROR)) {
                drawable = R.drawable.p_16_dd_parts_mle_mirror_2nd_disable;
            } else if (selectedTheme.equalsIgnoreCase("Manual")) {
                drawable = R.drawable.p_16_dd_parts_mle_manual_2nd_disable;
            }
            setImageResource(drawable);
        } else {
            if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.SKY) || selectedTheme.equalsIgnoreCase(ThemeSelectionController.SIL) || selectedTheme.equalsIgnoreCase(ThemeSelectionController.TEXTURE)) {
                drawable = R.drawable.p_16_dd_parts_mle_any_normal;
            } else if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.SOFTFILTER)) {
                drawable = R.drawable.p_16_dd_parts_mle_softfilter_2nd_normal;
            } else if (selectedTheme.equalsIgnoreCase("Rotation")) {
                drawable = R.drawable.p_16_dd_parts_mle_rotation_2nd_normal;
            } else if (selectedTheme.equalsIgnoreCase(ThemeSelectionController.MIRROR)) {
                drawable = R.drawable.p_16_dd_parts_mle_mirror_2nd_normal;
            } else if (selectedTheme.equalsIgnoreCase("Manual")) {
                drawable = R.drawable.p_16_dd_parts_mle_manual_2nd_normal;
            }
            setImageResource(drawable);
        }
        AppLog.exit(this.TAG, AppLog.getMethodName());
