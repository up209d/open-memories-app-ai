...
public class DriveModeController extends ShootingModeController {
    ...
    public void setValue(String tag, String value) {
        ...
        if (tag.equals(DRIVEMODE)) {
            ...
            if (value.equals(SELF_TIMER)) {
                ...
                params.setDriveMode(BURST);
                params.setBurstDriveSpeed(this.mBackupUtil.getPreferenceString(BaseBackUpKey.ID_DRIVE_MODE_BURST_SPEED, BURST_SPEED_LOW));
            } else {
                params.setDriveMode(SINGLE);
            }
            ...
            } else if (value.equals(BURST_SPEED_LOW)) {
                params.setBurstDriveSpeed(BURST_SPEED_LOW);
                this.mBackupUtil.setPreference(BaseBackUpKey.ID_DRIVE_MODE_BURST_SPEED, BURST_SPEED_LOW);
            }
        ...
        if (isMovieMode()) {
            ...
            if (time == 0) {
                ...
                if (isSupported(BURST)) {
                    ...
                    } else if (isSupported(BURST_SPEED_LOW) && speed.equals(BURST_SPEED_LOW)) {
                        value = BURST_SPEED_LOW;
                    }
                }
            } else if (time == 10) {
                if (burst_num == 5) {
    ...
    private void setDriveModeWithOption(String v) {
        ...
        if (v.equals(BURST_SPEED_HIGH)) {
            ...
            this.mBackupUtil.setPreference(BaseBackUpKey.ID_DRIVE_MODE_BURST_SPEED, BURST_SPEED_MID);
        } else if (v.equals(BURST_SPEED_LOW)) {
            this.mBackupUtil.setPreference(BaseBackUpKey.ID_DRIVE_MODE_BURST_SPEED, BURST_SPEED_LOW);
        }
    ...
    private String getDriveModeForMenu(String v) {
        ...
        if (v.equals(BURST_SPEED_HIGH) || v.equals(BURST_SPEED_MID) || v.equals(BURST_SPEED_LOW)) {
            return BURST;
        }
    ...
    public void setBackUpkey() {
        ...
        if (isSupported(speed)) {
            ...
            } else if (isSupported(BURST_SPEED_LOW)) {
                preference_s = BURST_SPEED_LOW;
            } else {
                preference_s = BURST_SPEED_HIGH;
    ...
    public List<String> getSupportedValue(String tag, int mode) {
        ...
        if (tag.equals(DRIVEMODE)) {
            ...
            if (isSupported(BURST_SPEED_LOW, mode)) {
                list.add(BURST_SPEED_LOW);
            }
        }
        return list;
    }
    ...
    protected boolean isSupported(String value, int mode) {
        ...
        if (!value.equals(BURST_SPEED_LOW) || (speeds = params.getSupportedBurstDriveSpeeds()) == null || 1 >= speeds.size()) {
            return false;
        }
        boolean ret8 = speeds.contains(BURST_SPEED_LOW);
        return ret8;
    }
    ...
    public boolean isAvailable(String value) {
        ...
        if (!value.equals(BURST_SPEED_LOW)) {
            return false;
        }
        boolean ret11 = AvailableInfo.isAvailable((Camera.Parameters) p.first, (CameraEx.ParametersModifier) p.second, SET_BURST_FUNCTION, BURST_SPEED_LOW);
        return ret11;
    }
