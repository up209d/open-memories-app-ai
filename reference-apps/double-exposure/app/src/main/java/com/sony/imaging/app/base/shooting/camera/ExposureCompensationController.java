...
public class ExposureCompensationController extends AbstractController implements CompensationControllable {
    ...
    private static boolean mTurnedEvDial = false;
    ...
    protected void createSupported() {
        ...
        if (mSupportedExposureCompensations == null) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETMINEXPOSURECOMPENSATION).append(mRawMinExposureCompensation).append(", ").append(LOG_MSG_GETMAXEXPOSURECOMPENSATION).append(mRawMaxExposureCompensation);
    ...
    public int getExposureCompensationIndex() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETEXPOSURECOMPENSATION).append(index);
        Log.i(TAG, STRBUILD.toString());
        return index;
    }
    ...
    public float getExposureCompensationStep() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETEXPOSURECOMPENSATIONSTEP).append(step);
        Log.i(TAG, STRBUILD.toString());
        return step;
    }
    ...
    public void setExposureCompensation(int value) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETEXPOSURECOMPENSATION).append(value);
        Log.i(TAG, STRBUILD.toString());
    ...
    public boolean setEvDialValue(int position, Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        boolean ret = false;
        if (mHasEvDial) {
            if (isExposureCompensationAvailable() && isModeDialAvailable()) {
    ...
    private class ExposureCompensationListener implements NotificationListener {
        ...
        public void onNotify(String tag) {
            if (ExposureCompensationController.mHasEvDial && ExposureCompensationController.mTurnedEvDial && (CameraNotificationManager.SCENE_MODE.equals(tag) || CameraNotificationManager.ISO_SENSITIVITY.equals(tag) || CameraNotificationManager.REC_MODE_CHANGED.equals(tag))) {
