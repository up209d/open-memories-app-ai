...
public class DoubleExposureKeyIcon extends ActiveImage {
    private final String TAG;

    public DoubleExposureKeyIcon(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void refresh() {
        ...
        if (DoubleExposureUtil.getInstance().getCurrentShootingScreen().equalsIgnoreCase(DoubleExposureConstant.FIRST_SHOOTING)) {
            if (1 == Environment.getVersionOfHW()) {
                setImageResource(R.drawable.p_16_dd_parts_key_sk2);
            } else {
                setImageResource(R.drawable.p_16_dd_parts_key_delete);
            }
        } else {
