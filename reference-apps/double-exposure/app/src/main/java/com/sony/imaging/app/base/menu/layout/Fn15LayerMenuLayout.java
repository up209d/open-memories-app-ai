...
public class Fn15LayerMenuLayout extends Fn15LayerAbsLayout {
    ...
    public void onResume() {
        ...
        if (itemList == null || itemList.size() < 2 || !isKeyHandled(action)) {
            openPreviousMenu();
            return;
        }
        ...
        if (this.mService.hasSupportedSubArray(curItemId)) {
            ...
            if (action == 3 || action == 4) {
                ...
                if (PictureEffectController.MODE_MINIATURE.equals(curItemId)) {
        ...
        if (adapter != null) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (item.selected != null) {
                    item.selected.setAlpha(255);
                }
                if (item.unselected != null) {
                    item.unselected.setAlpha(255);
                }
            }
        }
    }
    ...
    public int pushedCenterKey() {
        AdapterItem item = this.mOptionBeltView.getVisibility() == 0 ? (AdapterItem) this.mOptionBeltView.getSelectedItem() : (AdapterItem) this.mMainBeltView.getSelectedItem();
        if (item.valid) {
    ...
    public class Fn15LayerAdapter extends ArrayAdapter<AdapterItem> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (holder != null) {
                ...
                if (item.unselected != null) {
                    item.unselected.setAlpha(item.valid ? 255 : 128);
                }
            ...
            this.mUpdater = new DisplayMenuItemsMenuLayout.MenuUpdater(new Handler()) { // from class: com.sony.imaging.app.base.menu.layout.Fn15LayerMenuLayout.4
                ...
                public void run() {
                    if (Fn15LayerMenuLayout.this.mMainBeltView != null) {
                        ...
                        if (ckm.isCustomFuncKeyExist(ICustomKey.CATEGORY_MENU, CustomizableFunction.SubNext) && Fn15LayerMenuLayout.this.mService.hasSubArray(itemId)) {
