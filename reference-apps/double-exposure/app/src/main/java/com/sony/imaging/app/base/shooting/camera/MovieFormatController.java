...
public class MovieFormatController extends AbstractController {
    ...
    public static class Settings {
        ...
        public void setRecSetting(String value) {
            String key = MovieFormatController.AVCHD.equals(get(KEY_FORMAT)) ? KEY_REC_SETTING_AVCHD : KEY_REC_SETTING_MP4;
            set(key, value);
        }
    ...
    public void setValue(String tag, String value) {
        ...
        if (Environment.isMovieAPISupported()) {
            ...
            if (MOVIE_RECORD_SETTING.equals(tag) && (profile = appRecordSettingToPfProfile(value)) != null) {
                String key = AVCHD.equals(getValue(MOVIE_FORMAT)) ? Settings.KEY_REC_SETTING_AVCHD : Settings.KEY_REC_SETTING_MP4;
    ...
    public static void createSupportedProfileTable(MediaRecorder.Parameters supported) {
        if ((sAvchdProfileTable == null || sMp4ProfileTable == null) && supported != null) {
            List<MediaRecorder.CamcorderProfile> supportedList = supported.getSupportedCamcoderProfiles();
    ...
    public void onGetInitRecorderParameters(MediaRecorder.Parameters params) {
        if (Environment.isMovieAPISupported()) {
            ...
            if (format != null) {
                ...
                Log.i(TAG, sBuilder.toString());
                params.setOutputFormat(appFormatToPF(format));
            } else if (Environment.isMovieAPISupported()) {
                format = pfFormatToApp(this.mCamSet.getRecorderParameters().getOutputFormat());
            }
            String recordSet = this.mUserSetting.get(AVCHD.equals(format) ? Settings.KEY_REC_SETTING_AVCHD : Settings.KEY_REC_SETTING_MP4);
            if (recordSet != null) {
    ...
    public static MediaRecorder.CamcorderProfile appRecordSettingToPfProfile(String value) {
        ...
        if (sAvchdProfileTable != null) {
            MediaRecorder.CamcorderProfile profile2 = sAvchdProfileTable.get(value);
            profile = profile2;
        }
        if (profile == null && sMp4ProfileTable != null) {
            MediaRecorder.CamcorderProfile profile3 = sMp4ProfileTable.get(value);
            return profile3;
        }
        return profile;
    }
    ...
    public List<String> getAvailableValue(String tag) {
        ...
        if (supporteds != null) {
            ...
            if (MOVIE_FORMAT.equals(tag)) {
                ...
                String fileFormat = getValue(MOVIE_FORMAT);
                HashMap<String, MediaRecorder.CamcorderProfile> table = AVCHD.equals(fileFormat) ? sAvchdProfileTable : sMp4ProfileTable;
