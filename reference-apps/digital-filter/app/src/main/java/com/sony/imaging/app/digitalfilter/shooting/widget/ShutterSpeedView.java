...
public class ShutterSpeedView extends DigitView {
    ...
    private static final int MARGIN_TOP_ = 374;
    private static final int STD_L = 36;
    private static final int STD_S = 28;
    ...
    private void setValue(Pair<Integer, Integer> ss) {
        ...
        if (CameraSetting.isShutterSpeedBulb(ss)) {
            ...
            if (value < THRESHODL_ONE_OR_SMALL) {
                ...
                if (((Integer) ss.first).intValue() / ((Integer) ss.second).intValue() < 0.01010101f) {
                    isSmall = true;
                }
                if (((Integer) ss.first).intValue() / ((Integer) ss.second).intValue() < 1.25E-4f) {
                    isMoreSmall = true;
                }
            } else {
        ...
        if (!this.mFnMode) {
            setVisibility(4);
        } else if (getTextSize() == 36.0f) {
            if (!isSmall) {
                setVisibility(0);
            } else {
                setVisibility(4);
            }
        } else if (getTextSize() == 28.0f) {
            if (!isSmall || isMoreSmall) {
                setVisibility(4);
            } else {
                setVisibility(0);
            }
