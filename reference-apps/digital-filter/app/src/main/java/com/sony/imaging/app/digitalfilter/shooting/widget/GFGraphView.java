...
public class GFGraphView extends View implements NotificationListener {
    ...
    public GFGraphView(Context context, AttributeSet attrs) {
        ...
        if (BOTH_FILTER_HIST.equalsIgnoreCase(this.mMode)) {
            ...
            if (!GFCommonUtil.getInstance().isLayerSetting() && !GFEEAreaController.getInstance().isLand()) {
                this.mIndex = 0;
            } else {
        ...
        if (getBackground() == null) {
            Drawable histy = getResources().getDrawable(android.R.drawable.quickcontact_badge_overlay_normal_light);
            setBackgroundDrawable(histy);
            histy.setCallback(null);
        }
        getBackground().setAlpha(BASE_INDEX);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (BASE_HIST.equalsIgnoreCase(this.mMode)) {
            if (GFCommonUtil.getInstance().isLayerSetting() && GFCommonUtil.getInstance().isLand()) {
                this.paint.setColor(-30720);
            } else {
                this.paint.setColor(this.mColor);
            }
