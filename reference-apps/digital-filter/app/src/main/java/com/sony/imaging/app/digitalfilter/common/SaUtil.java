...
public class SaUtil {
    ...
    public static boolean isAVIP() {
        return GFCommonUtil.getInstance().isAVIP();
    }
    ...
    public static void dumpDeviceBuffer(DeviceBuffer devBuf) {
        ByteBuffer buf = ByteBuffer.allocateDirect(512);
        buf.order(ByteOrder.nativeOrder());
        devBuf.read(buf, 512, 0);
        for (int i = 0; i < 512; i += 4) {
            Log.i(TAG, "debug: 0x" + Integer.toHexString(i) + " : " + String.format("%08x", Integer.valueOf(buf.getInt())));
        }
    }
