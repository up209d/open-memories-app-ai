...
public class GFStableLayout extends StableLayout implements NotificationListener {
    ...
    private static BorderView mBorderView;
    ...
    static {
        ...
        mBorderView = null;
        TAGS = new String[]{DisplayModeObserver.TAG_DEVICE_CHANGE, DisplayModeObserver.TAG_DISPMODE_CHANGE, DisplayModeObserver.TAG_YUVLAYOUT_CHANGE, DisplayModeObserver.TAG_VIEW_PATTERN_CHANGE, CameraNotificationManager.HISTOGRAM_UPDATE, GFConstants.UPDATE_APPSETTING, GFConstants.CAMERA_SETTING_CHANGED, GFConstants.TAG_GYROSCOPE};
    }

    @Override // com.sony.imaging.app.base.shooting.layout.StableLayout, com.sony.imaging.app.fw.FakeFragment
    public void onResume() {
        super.onResume();
        if (GFFilterSetController.getInstance().getValue().equalsIgnoreCase(GFFilterSetController.TWO_AREAS)) {
            if (GFSettingMenuLayout.getCurrentArea().equals("Layer3Settings")) {
                GFSettingMenuLayout.disable3rdArea();
            }
            if (GFEEAreaController.getInstance().getValue().equals(GFEEAreaController.LAYER3)) {
                GFEEAreaController.getInstance().setValue(GFEEAreaController.EE, GFEEAreaController.LAYER3);
            }
        }
        int borderId = 0;
    ...
    protected void createView() {
        ...
        handleInfoIcon(device);
    }

    private void handleInfoIcon(int device) {
        ...
        if (this.mCurrentMainView != null && (1 == displayMode || device == 1)) {
            ImageView wifiIcon = (ImageView) this.mCurrentMainView.findViewById(R.id.gf_parts_wifi_img);
            wifiIcon.setVisibility(4);
        }
        if (this.mCurrentMainView != null && (1 == displayMode || device == 1)) {
            ImageView logIcon = (ImageView) this.mCurrentMainView.findViewById(R.id.logout_icon);
            GFCommonUtil.getInstance().updateLogIcon(logIcon);
        }
        mBorderView = (BorderView) this.mCurrentMainView.findViewById(R.id.border_view);
        if (3 == displayMode) {
            mBorderView.setVisibility(4);
        } else {
            mBorderView.setVisibility(0);
        }
    }
    ...
    public void onNotify(String tag) {
        ...
        if (GFConstants.TAG_GYROSCOPE.equals(tag) || GFConstants.CAMERA_SETTING_CHANGED.equals(tag) || DisplayModeObserver.TAG_VIEW_PATTERN_CHANGE.equals(tag)) {
