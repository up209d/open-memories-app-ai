...
public class CreativeStyleController extends AbstractController {
    ...
    private static final int PF_VER_INVALID_CREATIVE_STYLE_OPTION_0_SET = 7;
    ...
    private static StringBuilderThreadLocal builder = new StringBuilderThreadLocal();
    private static CreativeStyleOptions tmpOption = new CreativeStyleOptions(0, 0, 0);
    private static StringBuilderThreadLocal mBackUpKeyBuilder = new StringBuilderThreadLocal();
    private static StringBuilderThreadLocal mflattenBuilder = new StringBuilderThreadLocal();
    ...
    public CreativeStyleController() {
        setController(this);
    }

    /* loaded from: classes.dex */
    public static class CreativeStyleOptions {
        private static StringBuilderThreadLocal mStrBuilder = new StringBuilderThreadLocal();
        public int contrast;
        ...
        public String toString() {
            StringBuilder strBuilder = mStrBuilder.get();
            strBuilder.replace(0, strBuilder.length(), CreativeStyleController.LOG_CONTRAST).append(this.contrast).append(CreativeStyleController.LOG_SATURATION).append(this.saturation).append(CreativeStyleController.LOG_SHARPNESS).append(this.sharpness);
    ...
    public void setDetailValue(String mode, Object obj, Pair<Camera.Parameters, CameraEx.ParametersModifier> p) {
        ...
        StringBuilder strBuilder = builder.get();
        strBuilder.replace(0, strBuilder.length(), LOG_SET_DETAIL_VALUE).append("(").append(mode).append(LOG_MSG_ROUND_BRACKET_R);
        strBuilder.append(option.toString());
    ...
    public Object getDetailValue() {
        return getDetailValueFromCamera(this.mCamSet.getParameters());
    }

    protected CreativeStyleOptions getDetailValueFromCamera(Pair<Camera.Parameters, CameraEx.ParametersModifier> getParam) {
        CameraEx.ParametersModifier param = (CameraEx.ParametersModifier) getParam.second;
        return new CreativeStyleOptions(param.getContrast(), param.getSaturation(), param.getSharpness());
    }
    ...
    public Object getDetailValue(String value) {
        if (value != null) {
            ...
            if (value.equals(((CameraEx.ParametersModifier) getParam.second).getColorMode())) {
                return getDetailValueFromCamera(getParam);
            }
        }
        return getDetailValueFromBackUp(value);
    }

    public CreativeStyleOptions getDetailValueFromBackUp(String mode) {
        tmpOption.set(0, 0, 0);
    ...
    private String getBackUpKey(String mode) {
        StringBuilder backUpKeyBuilder = mBackUpKeyBuilder.get();
        backUpKeyBuilder.replace(0, backUpKeyBuilder.length(), "ID_CREATIVESTYLE_");
        if ("standard".equals(mode)) {
            backUpKeyBuilder.append("STD");
        } else if (VIVID.equals(mode)) {
            backUpKeyBuilder.append("VIVID");
        } else if (NEUTRAL.equals(mode)) {
            backUpKeyBuilder.append("NEUTRAL");
        } else if (CLEAR.equals(mode)) {
            backUpKeyBuilder.append("CLEAR");
        } else if (DEEP.equals(mode)) {
            backUpKeyBuilder.append("DEEP");
        } else if (LIGHT.equals(mode)) {
            backUpKeyBuilder.append("LIGHT");
        } else if ("portrait".equals(mode)) {
            backUpKeyBuilder.append("PORT");
        } else if ("landscape".equals(mode)) {
            backUpKeyBuilder.append("LAND");
        } else if ("sunset".equals(mode)) {
            backUpKeyBuilder.append("SUNSET");
        } else if (NIGHT.equals(mode)) {
            backUpKeyBuilder.append(SceneRecognitionObserver.SCENE_MODE_NIGHT);
        } else if (RED_LEAVES.equals(mode)) {
            backUpKeyBuilder.append("AUTUMN");
        } else if (MONO.equals(mode)) {
            backUpKeyBuilder.append("MONO");
        } else if (SEPIA.equals(mode)) {
            backUpKeyBuilder.append("SEPIA");
        } else {
            Log.e(TAG, LOG_ERROR_BACKUP);
        }
        backUpKeyBuilder.append("_OPTION");
        return backUpKeyBuilder.toString();
    }
    ...
    public boolean isUnavailableSceneFactor(String tag) {
        ...
        StringBuilder strBuilder = builder.get();
        Log.w(TAG, strBuilder.replace(0, tag.length(), tag).append(LOG_INVALID_TAG).append("setColorMode").toString());
        return false;
    }
    ...
    protected synchronized void updateDetailValue() {
        ...
        if (updateDetailValue(setParam)) {
            ...
            StringBuilder strBuilder = builder.get();
            strBuilder.replace(0, strBuilder.length(), LOG_GET_DETAIL_VALUE).append("(").append(((CameraEx.ParametersModifier) currentParam.second).getColorMode()).append(LOG_MSG_ROUND_BRACKET_R).append(tmpOption.toString());
            Log.i(TAG, strBuilder.toString());
        }
    }
    ...
    protected boolean updateDetailValue(Pair<Camera.Parameters, CameraEx.ParametersModifier> param) {
        ...
        StringBuilder strBuilder = builder.get();
        strBuilder.replace(0, strBuilder.length(), LOG_SET_DETAIL_VALUE).append("(").append(mode).append(LOG_MSG_ROUND_BRACKET_R);
        strBuilder.append(tmpOption.toString());
        Log.i(TAG, strBuilder.toString());
        return true;
    }
    ...
    protected String flatten(CreativeStyleOptions option) {
        StringBuilder flattenBuilder = mflattenBuilder.get();
        flattenBuilder.replace(0, flattenBuilder.length(), "contrast").append("=").append(option.contrast).append(";").append("saturation").append("=").append(option.saturation).append(";").append("sharpness").append("=").append(option.sharpness);
        return flattenBuilder.toString();
    }
