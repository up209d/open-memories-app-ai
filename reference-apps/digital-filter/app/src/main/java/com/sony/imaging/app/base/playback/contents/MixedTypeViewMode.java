...
public abstract class MixedTypeViewMode extends ViewMode {
    ...
    public static final String createClauseToSelectSomeValue(String column, int count) {
        ...
        StringBuilder builder2 = builder.replace(0, builder.length(), LogHelper.MSG_OPEN_BRACKET).append(column).append(ViewMode.EQUAL_ARGS);
        for (int i = 1; i < count; i++) {
            builder2.append(ViewMode.OR).append(column).append(ViewMode.EQUAL_ARGS);
        }
        builder2.append(LogHelper.MSG_CLOSE_BRACKET);
    ...
    public synchronized int findContents(SearchInfo info) {
        ...
        if (folderNumber < 0 || fileNumber < 0) {
            ...
            Log.i(TAG, LogHelper.getScratchBuilder(MSG_SEARCH_CONTENTS).append(info.folderNumber).append(", ").append(info.fileNumber).append(" : ").append(result).toString());
        }
        return result;
    }
