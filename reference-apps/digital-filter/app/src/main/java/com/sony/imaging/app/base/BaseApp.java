...
public abstract class BaseApp extends AppRoot {
    ...
    public static final String DA_MODE_TO_OPEN_MENU = "DA_MODE_TO_OPEN_MENU";
    ...
    private static final String VERSION = "Release_20161206_01";
    ...
    public static final String[] PULLING_BACK_KEYS_FOR_SHOOTING_ON_ACTION_CAM = {AppInfo.KEY_USB_CONNECT, AppInfo.KEY_PLAY};
    ...
    public static final String[] PULLING_BACK_KEYS_FOR_PLAYBACK_ON_ACTION_CAM = {AppInfo.KEY_S2, AppInfo.KEY_S1_1, AppInfo.KEY_S1_2, AppInfo.KEY_MOVREC, AppInfo.KEY_MODE_DIAL, AppInfo.KEY_USB_CONNECT, AppInfo.KEY_PLAY};
    ...
    protected String DEFAULT_BOOT_MODE = "DefaultBootMode";
    protected boolean mIgnoreExclusiveCaution = true;
    protected boolean mIgnoreUnsupportedCaution = true;
    protected boolean mIgnoreLowerPriorityCautionChange = true;
    private CautionUtilityClass.CautionCallback cautionProcessing = new CautionUtilityClass.CautionCallback() { // from class: com.sony.imaging.app.base.BaseApp.1
        ...
        public void onCallback(int[] cautionId) {
            ...
            if (BaseApp.this.mIgnoreLowerPriorityCautionChange && CautionProcessingFunction.getMaxPriorityId(cautionId).equals(CautionProcessingFunction.getMaxPriorityId(currentCautionId))) {
                Log.i(BaseApp.TAG, "[END]onCallback max prior cautionId NOT changed");
                return;
            }
            int cnt = 0;
            for (int i : cautionId) {
    ...
    public boolean isHousingSupported() {
        return false;
    }
    ...
    public boolean useMovieSettingDatabase() {
        return (getSupportingRecMode() & 10) != 0;
    }
    ...
    protected Bundle getStartAppData() {
        ...
        Bundle bundle = new Bundle();
        int d2ATransitionMode = BaseProperties.getD2ATransitionMode();
        bundle.putInt(DA_MODE_TO_OPEN_MENU, d2ATransitionMode);
        Log.v(TAG, "getStartAppData() getD2ATransitionMode: " + d2ATransitionMode);
        return bundle;
    }

    public BaseApp() {
        Log.i(TAG, "BaseApp version Release_20161206_01");
        ...
        CameraSetting.registController(ISOAutoMinSSController.getName(), ISOAutoMinSSController.class);
        ...
        CameraSetting.registController(WindNoiseReductionController.getName(), WindNoiseReductionController.class);
        CameraSetting.registController(AudioRecordingController.getName(), AudioRecordingController.class);
        CameraSetting.registController(AngleController.getName(), AngleController.class);
        CameraSetting.registController(FlipController.getName(), FlipController.class);
        CameraSetting.registController(SilentShutterController.getName(), SilentShutterController.class);
        CameraSetting.registController(RawRecModeController.getName(), RawRecModeController.class);
    }
    ...
    public void onBoot(BootFactor factor) {
        ...
        if (creator.isInheritRawRecMode()) {
            RawRecModeController.inheritRawRecModeFromDiadem();
        }
        if (!isSameVersion()) {
            _needUpdatePreference();
        }
        SubLcdManager.getInstance().init(getApplicationContext(), (ViewGroup) getView());
        selectShootingModeOnBoot();
        if (Environment.isNewBizDeviceActionCam() && Environment.hasSubLcd()) {
            setData(SubLcdStableLayout.KEY_START_FROM_TOP, true);
        }
        BackupReader.reset();
        ...
        if (BluetoothController.getInstance().isDeviceSupportedBluetooth()) {
            BluetoothNotificationManager.getInstance().resume(this);
        }
        if (TouchManager.getInstance().isDeviceSupportedTouch()) {
            TouchManager.getInstance().resume();
        }
    }
    ...
    public void onShutdown() {
        SubLcdManager.getInstance().terminate();
        super.onShutdown();
        AudioSetting.getInstance().terminate();
        if (GpsController.getInstance().isDeviceSupportedGps()) {
            GpsNotificationManager.getInstance().pause(this);
        }
        MediaNotificationManager.getInstance().pause(this);
        TemperatureManager.getInstance().pause(this);
        if (useMovieSettingDatabase()) {
        ...
        if (Environment.isSupportingDefaultBootMode()) {
            BaseProperties.setD2ATransitionMode(0);
        }
        if (BluetoothController.getInstance().isDeviceSupportedBluetooth()) {
            BluetoothNotificationManager.getInstance().pause(this);
        }
        if (TouchManager.getInstance().isDeviceSupportedTouch()) {
            TouchManager.getInstance().pause();
        }
    }

    public static void rememberDefaultBootModeOnPullingBack() {
        if (Environment.isSupportingDefaultBootMode()) {
            ExecutorCreator creator = ExecutorCreator.getInstance();
            String bootMode = creator.recModeTodefaultBootMode(creator.getRecordingModeFromBackup());
    ...
    public void selectShootingModeOnBoot() {
        if (getSupportingRecMode() != 0) {
            ExecutorCreator creator = ExecutorCreator.getInstance();
            if (Environment.isSupportingDefaultBootMode()) {
                String storedBootMode = BackUpUtil.getInstance().getPreferenceString(BaseBackUpKey.ID_AK_DEFAULT_MODE, "movie");
                String defaultBootMode = storedBootMode;
                switch (BaseProperties.getD2ATransitionMode()) {
                    case 1:
                        defaultBootMode = "loop";
                        break;
                    case 2:
                        defaultBootMode = "movie";
                        break;
                    case 3:
                        defaultBootMode = "photo";
                        break;
                    case 4:
                        defaultBootMode = "interval";
                        break;
                    case 10:
                        if ("photo".equals(defaultBootMode) || "interval".equals(defaultBootMode)) {
                            defaultBootMode = "movie";
                            break;
                        }
                        break;
                    case 11:
                        if ("movie".equals(defaultBootMode) || "loop".equals(defaultBootMode) || "live".equals(defaultBootMode)) {
                            defaultBootMode = "photo";
                            break;
                        }
                        break;
                }
                Log.i(TAG, "default boot mode : " + storedBootMode + " -> " + defaultBootMode);
                if (!storedBootMode.equals(defaultBootMode)) {
                    BackUpUtil.getInstance().setPreference(BaseBackUpKey.ID_AK_DEFAULT_MODE, defaultBootMode);
                }
                int recMode = creator.defaultBootModeToRecMode(defaultBootMode);
                if (recMode != 0) {
                    creator.setRecordingMode(recMode, null);
                    if (Environment.isNewBizDeviceActionCam() && Environment.hasSubLcd()) {
                        removeData(this.DEFAULT_BOOT_MODE);
                        setData(this.DEFAULT_BOOT_MODE, Integer.valueOf(recMode));
                        return;
                    }
                    return;
                }
