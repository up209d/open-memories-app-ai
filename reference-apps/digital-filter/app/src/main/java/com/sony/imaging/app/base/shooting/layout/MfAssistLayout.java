...
public class MfAssistLayout extends Layout implements IModableLayout {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (this.mCurrentLayout != null) {
            ...
            if (this.mTouchArea != null) {
                this.mTouchArea.setLongClickable(false);
                TouchArea.OnTouchAreaListener touchAreaListener = getOnTouchAreaListener();
                this.mTouchArea.setTouchAreaListener(touchAreaListener);
                if (touchAreaListener instanceof TouchArea.OnTouchAreaDoubleTapListener) {
                    this.mTouchArea.setTouchAreaDoubleTapListener((TouchArea.OnTouchAreaDoubleTapListener) touchAreaListener);
                }
            }
    ...
    /* loaded from: classes.dex */
    public class MfAssistTouchListner implements TouchArea.OnTouchAreaListener, TouchArea.OnTouchAreaDoubleTapListener {
        ...
        public boolean onTouchDown(MotionEvent e) {
            ...
            FocusMagnificationController controller = FocusMagnificationController.getInstance();
            int actualMagnificationRatio = controller.getActualMagnificationRatio();
            if (actualMagnificationRatio != 100) {
                return false;
            }
            controller.rescheduleTimeout();
            return controller.setMagnifyingPositionDirectly((int) e.getRawX(), (int) e.getRawY());
        }
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (touchOperationVersion >= 2) {
                ...
                if (actualMagnificationRatio == 100) {
                    controller.rescheduleTimeout();
                    return controller.setMagnifyingPositionDirectly((int) e2.getRawX(), (int) e2.getRawY());
                }
                controller.rescheduleTimeout();
                controller.moveMagnifyingPositionDirectly((int) distanceX, (int) distanceY);
                return true;
            }
            return false;
        }
