...
public abstract class SingleLayoutBase extends OptimizedImageLayoutBase implements IModableLayout, TouchArea.OnTouchAreaListener {
    ...
    private static final Rect YUV_LAYOUT_NORMAL = new Rect(0, 0, AppRoot.USER_KEYCODE.WATER_HOUSING, CatchLightPlayBackLayout.mLayoutParams_faceZoom__height);
    ...
    public boolean onFlick(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (velocityX < PortraitBeautyConstants.INVALID_APERTURE_VALUE) {
            nextContents();
            return true;
        }
        if (velocityX > PortraitBeautyConstants.INVALID_APERTURE_VALUE) {
