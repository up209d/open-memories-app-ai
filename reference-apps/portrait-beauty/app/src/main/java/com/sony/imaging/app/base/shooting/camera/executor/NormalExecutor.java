...
public class NormalExecutor extends ShootingExecutor {
    ...
    protected boolean isSelfTimerExecuting = false;
    ...
    public void myRelease() {
        if (this.isSelfTimerExecuting) {
            myCancelSelfTimerShutter();
        }
        tryRelease();
    }
    ...
    public class NormalShutterCb extends ShootingExecutor.MyShutterCb {
        ...
        public void onShutter(int status, CameraEx cameraEx) {
            ...
            Log.d(NormalExecutor.TAG, "onShutter");
            int type = NormalExecutor.this.getShutterType();
            NormalExecutor.this.isSelfTimerExecuting = false;
            if (status != 0) {
    ...
    public void myStartSelfTimerShutter() {
        ...
        enableTermination(false);
        this.isSelfTimerExecuting = ((ICaptureAdapater) this.mAdapter).startSelfTimerShutter();
        if (CameraSetting.getInstance().isShutterSpeedBulb()) {
            isBulbOpened = true;
        }
    }
    ...
    public void myCancelSelfTimerShutter() {
        ...
        StringBuilder builder = sStrBuilderPool.get();
        builder.replace(0, builder.length(), LOG_CANCEL_SELF_TIMER).append(LOG_STATUS).append(status);
        Log.i(TAG, builder.toString());
        if (status) {
            this.isSelfTimerExecuting = false;
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void setTempRemoteControl(int status) {
