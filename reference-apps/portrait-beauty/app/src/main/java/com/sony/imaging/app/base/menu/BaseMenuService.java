...
public class BaseMenuService {
    public static final String BACK = "back_transition";
    ...
    private static Class<? extends CautionID> mCatuionIdClass;
                ...
                if (setItem != null) {
                    ...
                    if (setItem.configClass == null) {
                        ...
                        if (supportedList != null && supportedList.size() != 0) {
                            retList.add(itemId);
                        } else if (MenuTable.BACK_TRANSITION.equals(setItem.execType)) {
                            retList.add(itemId);
                        }
                    }
                }
    ...
    protected ArrayList<String> getSupportedValueItemList(String itemId, int mode) {
        ...
        while (i$.hasNext()) {
            ...
            } else if (BACK.equals(menuItemID)) {
                retList.add(menuItemID);
            }
        }
        return retList;
    ...
    public String getCurrentValue(String itemId) {
        ...
        try {
            ...
            boolean isCompensationValue = DEBUG;
            ...
            if (isCompensationValue && (controller instanceof CompensationControllable)) {
                ...
                if (f == PortraitBeautyConstants.INVALID_APERTURE_VALUE) {
    ...
    public static void setEachCautionIdClass(Class<? extends CautionID> clazz) {
        mCatuionIdClass = clazz;
        mCautionFieldPool.clear();
    }
    ...
    public int[] getMenuItemCautionId(String itemid) {
        ...
        if (this.mAccessor != null) {
            ...
            while (it.hasNext()) {
                ...
                if (cautionName.contains(StringBuilderThreadLocal.PERIOD)) {
    ...
    public boolean isMenuItemValid(String itemid) {
        MenuAccessor.DisplayMenuItem item = getDisplayItem(itemid);
        return item == null ? DEBUG : item.isValid;
    }
    ...
    public Integer getInteger(String itemId, String element) {
        MenuAccessor.DisplayMenuItem item = getDisplayItem(itemId);
        if (item != null && item.elements != null) {
            String value = item.elements.get(element);
            try {
    ...
    public String getPrevItemId(String currenItemId) {
        String parentItemId = getMenuItemId();
        String prevItemId = null;
        if (currenItemId != null) {
            if (parentItemId != null) {
                if (this.mAccessor != null) {
                    ArrayList<String> items = getSupportedItemList(parentItemId);
                    if (items != null) {
                        int size = items.size();
                        int currentItemIndex = items.indexOf(currenItemId);
                        if (currentItemIndex < 0) {
                            Log.w(TAG, "currenItemId is not contained");
                        } else if (currentItemIndex == 0) {
                            int previousItemIndex = size - 1;
                            prevItemId = items.get(previousItemIndex);
                        } else {
                            int previousItemIndex2 = currentItemIndex - 1;
                            prevItemId = items.get(previousItemIndex2);
                        }
                    } else {
                        Log.w(TAG, "items is null");
                    }
                } else {
                    Log.w(TAG, "mAccessor is null");
                }
            } else {
                Log.w(TAG, "parentItemId does not exist");
            }
        } else {
            Log.w(TAG, "currenItemId is null");
        }
        Log.i(TAG, "prevItemId is " + prevItemId);
        return prevItemId;
    }

    public String getNextItemId(String currenItemId) {
        String parentItemId = getMenuItemId();
                ...
                if (this.mAccessor != null) {
                    ...
                    if (items != null) {
                        ...
                        if (currentItemIndex < 0) {
                            ...
                            nextItemId = items.get(nextItemIndex);
                        }
                    } else {
                        Log.w(TAG, "items is null");
                    }
        ...
        Log.i(TAG, "nextItemId is " + nextItemId);
        return nextItemId;
    }

    public List<String> getMenuItemRecMode(String itemid) {
        MenuAccessor.DisplayMenuItem dmi = getDisplayItem(itemid);
        if (dmi == null) {
            return null;
        }
    ...
    public void setMenuItemIdWithHistory(String itemid) {
        if (this.mAccessor != null) {
            this.mAccessor.updateMainMenuList(itemid);
            this.mAccessor.creteHistory(itemid);
        }
    }
}
