...
public class DisplayModeObserver extends NotificationManager {
    ...
    public void resume(boolean is4kOutputSupported) {
        ...
        if (mYUVRect != null) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_ONLAYOUTCHANGED).append(LOG_MSG_RECT).append(" ( ").append(mYUVRect.pxLeft).append(", ").append(mYUVRect.pxTop).append(" ) ").append(" - ").append(" ( ").append(mYUVRect.pxRight).append(", ").append(mYUVRect.pxBottom).append(" ) ");
            Log.i(TAG, STRBUILD.toString());
        }
    }
    ...
    public class OnVideoLayoutStatusListener implements DisplayManager.VideoLayoutStatusListener {
        ...
        public void onLayoutChanged(DisplayManager.VideoRect rect) {
            DisplayModeObserver.STRBUILD.replace(0, DisplayModeObserver.STRBUILD.length(), DisplayModeObserver.LOG_MSG_ONLAYOUTCHANGED).append(DisplayModeObserver.LOG_MSG_RECT).append(" ( ").append(rect.pxLeft).append(", ").append(rect.pxTop).append(" ) ").append(" - ").append(" ( ").append(rect.pxRight).append(", ").append(rect.pxBottom).append(" ) ");
            Log.i(DisplayModeObserver.TAG, DisplayModeObserver.STRBUILD.toString());
    ...
    public boolean start4kPlayback() {
        if (this.mIs4kOutputSupported) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_START_QFHD_FORMAT).append(this.m4kOutputStatus);
    ...
    public boolean stop4kPlayback() {
        if (this.mIs4kOutputSupported) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_STOP_QFHD_FORMAT).append(this.m4kOutputStatus);
            Log.d(TAG, STRBUILD.toString());
            if (1 != this.m4kOutputStatus && 5 != this.m4kOutputStatus) {
                this.mDisplayManager.stopQfhdFormatOutput();
