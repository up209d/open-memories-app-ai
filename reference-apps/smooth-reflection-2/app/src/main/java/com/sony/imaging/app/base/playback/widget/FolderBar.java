...
public class FolderBar extends ImageView {
    ...
    private int getTouchedArea(float xPos, float yPos) {
        ...
        float xEnd = SmoothReflectionConstants.INVALID_APERTURE_VALUE + btnWidth;
        ...
        if (xPos >= SmoothReflectionConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= SmoothReflectionConstants.INVALID_APERTURE_VALUE && yPos < yUpperEnd) {
            return 0;
        }
        if (xPos >= SmoothReflectionConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= yCenterStart && yPos < yCenterEnd) {
            return 1;
        }
        if (xPos < SmoothReflectionConstants.INVALID_APERTURE_VALUE || xPos >= xEnd || yPos < yLowerStart || yPos >= btnHeight) {
            return -1;
        }
        return 2;
    }
    ...
    public void setImagePressed(int direction) {
        setImagePressed(direction, DEFAULT_DURATION);
    }
