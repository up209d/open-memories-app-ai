...
public class FocusMagnificationController extends ShootingModeController {
    ...
    public boolean isAvailableInSettings() {
        ...
        this.mStrBuilder.replace(0, this.mStrBuilder.length(), MSG_IS_AVAILABLE_IN_SETTINGS).append(result);
        Log.d(TAG, this.mStrBuilder.toString());
        return result;
    }
    ...
    public int getTimeoutDurationInSettings() {
        ...
        this.mStrBuilder.replace(0, this.mStrBuilder.length(), MSG_GET_TIMEOUT_IN_SETTINGS).append(result);
        Log.d(TAG, this.mStrBuilder.toString());
        return result;
    }
    ...
    public Pair<Integer, Integer> convertCoordinateOnEEToApp(int x, int y) {
        ...
        if (ratio == 0) {
            ratio = 100;
        } else if (ratio == 100 && (supported = getSupportedValueInt(TAG_MAGNIFICATION_RATIO)) != null && supported.size() > 0) {
            ratio = this.mCamSet.getFocusMagnificationRatio(supported.get(0).intValue());
        }
        ...
        float power = 2000.0f / (ratio / 100.0f);
        ...
        return convertToStep((100 == ratio || current == null) ? Pair.create(Integer.valueOf(diffX), Integer.valueOf(diffY)) : Pair.create(Integer.valueOf(((Integer) current.first).intValue() + diffX), Integer.valueOf(((Integer) current.second).intValue() + diffY)));
    }
    ...
    protected Pair<Float, Float> getDistancePerUnitStep() {
        ...
        if (lowestRatio == 0) {
            frame = Pair.create(Float.valueOf(SmoothReflectionConstants.INVALID_APERTURE_VALUE), Float.valueOf(SmoothReflectionConstants.INVALID_APERTURE_VALUE));
        } else {
    ...
    public boolean isAvailable(String tag) {
        ...
        if (isSupportedByPf()) {
            ...
            if (ITEM_ID_FOCUS_MAGNIFICATION.equals(tag) && available) {
    ...
    protected Pair<Integer, Integer> movePositionInDigitalZoom() {
        ...
        float currentPosiX = SmoothReflectionConstants.INVALID_APERTURE_VALUE;
        float currentPosiY = SmoothReflectionConstants.INVALID_APERTURE_VALUE;
        ...
        if (this.mCamSet.mChangeZoomRatio) {
            ...
            if (movePositionCenter) {
                currentPosiX = SmoothReflectionConstants.INVALID_APERTURE_VALUE;
                currentPosiY = SmoothReflectionConstants.INVALID_APERTURE_VALUE;
            }
        }
