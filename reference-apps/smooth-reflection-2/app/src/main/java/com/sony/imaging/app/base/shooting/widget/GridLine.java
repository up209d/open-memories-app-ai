...
public class GridLine extends View {
    ...
    protected void refresh() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETGRIDLINE).append(this.mGridMode);
    ...
    private void xline(Canvas c) {
        c.drawLine(SmoothReflectionConstants.INVALID_APERTURE_VALUE, SmoothReflectionConstants.INVALID_APERTURE_VALUE, this.width, this.height, this.mPaint);
        c.drawLine(SmoothReflectionConstants.INVALID_APERTURE_VALUE, this.height, this.width, SmoothReflectionConstants.INVALID_APERTURE_VALUE, this.mPaint);
    }
    ...
    private void separateline(int separateWidthNum, int separateHeightNum, Canvas c) {
        ...
        for (int i = 1; i < separateWidthNum; i++) {
            c.drawLine(widthInterval * i, SmoothReflectionConstants.INVALID_APERTURE_VALUE, widthInterval * i, this.height, this.mPaint);
        }
        for (int i2 = 1; i2 < separateHeightNum; i2++) {
            c.drawLine(SmoothReflectionConstants.INVALID_APERTURE_VALUE, heightInterval * i2, this.width, heightInterval * i2, this.mPaint);
        }
    }
}
