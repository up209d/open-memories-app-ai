...
public class PictureQualityController extends ShootingModeController {
    ...
    private static List<String> createSupportedValueArray(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        builder.replace(0, builder.length(), LOG_MSG_GETSUPPORTEDPICTURESTORAGEFORMATS).append(formats);
    ...
    public void setValue(String tag, String value) throws IllegalArgumentException {
        ...
        if (command != null) {
            ...
            StringBuilder builder = STRBUILDS.get();
            builder.replace(0, builder.length(), LOG_MSG_SETPICTURESTORAGEFORMAT).append((String) command.first);
            Log.i(TAG, builder.toString());
            if (-1 != ((Integer) command.second).intValue()) {
                ((Camera.Parameters) params.first).setJpegQuality(((Integer) command.second).intValue());
                builder.replace(0, builder.length(), LOG_MSG_SETJPEGQUALITY).append(command.second);
                Log.i(TAG, builder.toString());
            }
    ...
    public List<String> getAvailableValue(String tag) {
        ...
        if (supporteds != null) {
            ...
            for (String mode : supporteds) {
                if (AvailableInfo.isAvailable("setPictureStorageFormat", mode)) {
                    availables.add(mode);
                }
            }
        }
    ...
    public boolean isUnavailableSceneFactor(String tag) {
        boolean b = isUnavailableAPISceneFactor("setPictureStorageFormat", null);
        return b;
    }
}
