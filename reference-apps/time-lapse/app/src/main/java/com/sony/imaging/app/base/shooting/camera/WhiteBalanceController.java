...
public class WhiteBalanceController extends AbstractController {
    ...
    protected static final String MSG_INH_ADJ_VALUES = "set adj 0 temporarily";
    ...
    private NotificationListener mListenerToResetABGM;
    ...
    String[] wbMode = {"auto", DAYLIGHT, SHADE, "cloudy-daylight", INCANDESCENT, FLUORESCENT_DAYLIGHT, WARM_FLUORESCENT, FLUORESCENT_DAYWHITE, FLUORESCENT_COOLWHITE, FLASH, "color-temp", CUSTOM};
    ...
    public WhiteBalanceController() {
        this.mListener = new ExposureModeChangedListener();
        this.mListenerToResetABGM = new ExposureModeChangedListenerToResetABGM();
        setController(this);
    }
    ...
    public void saveCustomWhiteBalance(String itemId) {
        ...
        if (Environment.isCustomWBOnePush()) {
            this.mCamSet.saveCWBAndUpdateCameraParams(customNo);
            ...
            option.lightBalance = ((CameraEx.ParametersModifier) p.second).getLightBalanceForWhiteBalance();
            option.colorComp = ((CameraEx.ParametersModifier) p.second).getColorCompensationForWhiteBalance();
            option.colorTemp = ((CameraEx.ParametersModifier) p.second).getColorTemperatureForWhiteBalance();
            setWBOptionToBackUp(itemId, option);
            return;
        }
        this.mCamSet.saveCustomWhiteBalance(customNo);
    }
    ...
    public void setDetailValue(Object obj) {
        ...
        Pair<Camera.Parameters, CameraEx.ParametersModifier> p = this.mCamSet.getEmptyParameters();
        boolean isSet = setWBOption(p, option);
    ...
    private boolean setWBOption(Pair<Camera.Parameters, CameraEx.ParametersModifier> p, WhiteBalanceParam option) {
        boolean isSet = false;
        List<String> lightBalanceSetting = getSupportedValue(SETTING_LIGHTBALANCE);
        List<String> compensationSetting = getSupportedValue(SETTING_COMPENSATION);
        if (lightBalanceSetting != null || compensationSetting != null) {
            if (lightBalanceSetting.contains(String.valueOf(option.lightBalance))) {
                ((CameraEx.ParametersModifier) p.second).setLightBalanceForWhiteBalance(option.lightBalance);
                isSet = true;
            }
            if (compensationSetting.contains(String.valueOf(option.colorComp))) {
        ...
        List<String> temperatureSetting = getSupportedValue(SETTING_TEMPERATURE);
        if (("color-temp".equals(mode) || temperatureSetting != null) && temperatureSetting.contains(String.valueOf(option.colorTemp))) {
    ...
    private WhiteBalanceParam getWBOption(Pair<Camera.Parameters, CameraEx.ParametersModifier> modeParam) {
        ...
        if ("color-temp".equals(mode) || CUSTOM.equals(mode)) {
        ...
        if (Environment.isCustomWBOnePush() && (CUSTOM.equals(mode) || "custom1".equals(mode) || "custom2".equals(mode) || "custom3".equals(mode))) {
            option.mInRange = ((CameraEx.ParametersModifier) this.mCamSet.getParameters().second).isCustomWhiteBalanceInRange();
        }
        return option;
    }
    ...
    public WhiteBalanceParam getWBOptionFromBackUp(String mode) {
        ...
        if ("color-temp".equals(mode)) {
            ...
            if (Environment.isCustomWBOnePush()) {
                option.mInRange = ((CameraEx.ParametersModifier) camera_param.second).isCustomWhiteBalanceInRange();
            }
        }
        option.lightBalance = lightBalance;
        option.colorComp = colorComp;
        option.colorTemp = colorTemp;
        return option;
    }
    ...
    protected String getBackUpKey(String mode, String option) {
        ...
        if ("auto".equals(mode)) {
            ...
            ret = "ID_WB_SHADE_";
        } else if ("cloudy-daylight".equals(mode)) {
            ret = "ID_WB_CLOUDY_";
        ...
        } else if ("color-temp".equals(mode)) {
    ...
    /* loaded from: classes.dex */
    private class ExposureModeChangedListenerToResetABGM implements NotificationListener {
        private static final String MSG_SET_ADJ_BACK = "re-set adj value";
        private final String[] tags;

        private ExposureModeChangedListenerToResetABGM() {
            this.tags = new String[]{CameraNotificationManager.SCENE_MODE};
        }
        ...
        public void onNotify(String tag) {
            if (CameraNotificationManager.SCENE_MODE.equals(tag)) {
                ...
                boolean isAvailable = WhiteBalanceController.this.isAvailable(WhiteBalanceController.WHITEBALANCE);
                if (isAvailable) {
                    String mode = WhiteBalanceController.this.getValue();
                    Log.i(WhiteBalanceController.TAG, MSG_SET_ADJ_BACK);
                    Pair<Camera.Parameters, CameraEx.ParametersModifier> p = WhiteBalanceController.this.mCamSet.getEmptyParameters();
                    ((CameraEx.ParametersModifier) p.second).setLightBalanceForWhiteBalance(WhiteBalanceController.this.mBackUp.getPreferenceInt(WhiteBalanceController.this.getBackUpKey(mode, WhiteBalanceController.LIGHT), 0));
                    ((CameraEx.ParametersModifier) p.second).setColorCompensationForWhiteBalance(WhiteBalanceController.this.mBackUp.getPreferenceInt(WhiteBalanceController.this.getBackUpKey(mode, WhiteBalanceController.COMP), 0));
                    ((CameraEx.ParametersModifier) p.second).tryIgnoreInhibit();
                ...
                CameraNotificationManager.getInstance().removeNotificationListener(WhiteBalanceController.this.mListenerToResetABGM);
            }
        }
    }
    ...
    public void onGetInitParameters(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        boolean isAvailable = isAvailable(WHITEBALANCE);
        if (!isAvailable) {
            Pair<Camera.Parameters, CameraEx.ParametersModifier> getParam = this.mCamSet.getParameters();
            if (((CameraEx.ParametersModifier) getParam.second).getLightBalanceForWhiteBalance() != 0 || ((CameraEx.ParametersModifier) getParam.second).getColorCompensationForWhiteBalance() != 0) {
                Log.w(TAG, MSG_INH_ADJ_VALUES);
                ((CameraEx.ParametersModifier) params.second).setLightBalanceForWhiteBalance(0);
                ((CameraEx.ParametersModifier) params.second).setColorCompensationForWhiteBalance(0);
                CameraNotificationManager.getInstance().setNotificationListener(this.mListenerToResetABGM);
            }
        }
    ...
    public void onGetTermParameters(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        CameraNotificationManager.getInstance().removeNotificationListener(this.mListenerToResetABGM);
    }
}
