...
public class ExposureCompensationController extends AbstractController implements CompensationControllable {
    ...
    private static final int PF_VERSION_OF_DX = 15;
    ...
    public void incrementExposureCompensation() {
        if (getSupportedValue(null) != null) {
    ...
    private static List<String> createSupportedValueArray(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        if (mRawMinExposureCompensation == 0 && mRawMaxExposureCompensation == 0) {
            return null;
        }
        ...
        if (getSupportedValue(null) != null) {
            if (CameraSetting.getPfApiVersion() >= 2) {
                ...
                if (ExposureModeController.MANUAL_MODE.equals(expMode)) {
                    ...
                    if (!"0".equals(isoValue)) {
                        ret = available.indexOf("0") != -1 ? false : false;
                    }
                }
            }
        } else {
    ...
    private void setExposureBiasModeOnForDX() {
        if (CameraSetting.getPfApiVersion() == 15 && ScalarProperties.getInt("model.category") == 1) {
            int index = getExposureCompensationIndex();
            Pair<Camera.Parameters, CameraEx.ParametersModifier> emptyParams = this.mCameraSetting.getEmptyParameters();
            ((Camera.Parameters) emptyParams.first).setExposureCompensation(0);
            synchronized (this.mCameraSetting) {
                this.mCameraSetting.getCamera().getNormalCamera().setParameters((Camera.Parameters) emptyParams.first);
                ((Camera.Parameters) this.mCameraSetting.getParameters().first).setExposureCompensation(0);
            }
            if (index != 0) {
    ...
    public class EvCacheInfo {
        ...
        public void init() {
            if (!Environment.isAllParameterInheritedByPF()) {
        ...
        public void saveCacheValue() {
            if (!Environment.isAllParameterInheritedByPF() && Environment.isMovieAPISupported() && !this.mUserSetting) {
                this.mValue = ExposureCompensationController.this.getExposureCompensationIndex();
            }
        }

        public void setCacheValue() {
            if (!Environment.isAllParameterInheritedByPF() && Environment.isMovieAPISupported() && !this.mUserSetting && this.mReopenFromMovie) {
