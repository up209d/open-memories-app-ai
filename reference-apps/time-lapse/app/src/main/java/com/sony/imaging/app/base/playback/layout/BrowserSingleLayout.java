        ...
        if (this.data != null) {
            ...
            if (touchEventAutoReview != null) {
                ...
                if (action.equals(EventParcel.FLICK)) {
                    if (touchEventAutoReview.mVelocityX > TimeLapseConstants.INVALID_APERTURE_VALUE) {
                        Log.v(MSG_AUTO_REVIEW_TOUCH, LogHelper.getScratchBuilder(action).append(MSG_DIRECTION_RIGHT).toString());
                        previousContents();
                    } else if (touchEventAutoReview.mVelocityX < TimeLapseConstants.INVALID_APERTURE_VALUE) {
                    ...
                    if (touchEventAutoReview.mDistanceX > TimeLapseConstants.INVALID_APERTURE_VALUE) {
                        Log.v(MSG_AUTO_REVIEW_TOUCH, LogHelper.getScratchBuilder(action).append(MSG_DIRECTION_RIGHT).toString());
                        previousContents();
                    } else if (touchEventAutoReview.mDistanceX < TimeLapseConstants.INVALID_APERTURE_VALUE) {
                        Log.v(MSG_AUTO_REVIEW_TOUCH, LogHelper.getScratchBuilder(action).append(MSG_DIRECTION_LEFT).toString());
                        nextContents();
                    }
                }
    ...
    public boolean onTouchUp(MotionEvent e, boolean isReleasedInside) {
        ...
        if (isReleasedInside) {
            ...
            if (displayMode == 5) {
                ...
                EventParcel touch = new EventParcel(null, e, null, isReleasedInside, TimeLapseConstants.INVALID_APERTURE_VALUE, TimeLapseConstants.INVALID_APERTURE_VALUE, TimeLapseConstants.INVALID_APERTURE_VALUE, TimeLapseConstants.INVALID_APERTURE_VALUE, viewRect);
