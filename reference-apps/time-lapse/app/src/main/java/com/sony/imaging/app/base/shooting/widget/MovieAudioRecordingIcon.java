...
public class MovieAudioRecordingIcon extends ActiveImage {
    ...
    public void refresh() {
        ...
        if (Environment.isMovieAPISupported()) {
            ...
            if (parameters != null && parameters.getMicrophoneEnable() && "on".equals(parameters.getMicrophoneWindNoiseReduction()) && ExecutorCreator.getInstance().isCameraModeMovie()) {
    ...
    public boolean isVisible() {
        if (!Environment.isMovieAPISupported() || !ExecutorCreator.getInstance().isCameraModeMovie()) {
            return false;
        }
        AudioManager.Parameters parameters = AudioSetting.getInstance().getAudioParameters();
        if (parameters != null && !parameters.getMicrophoneEnable()) {
