...
public class FolderBar extends ImageView {
    ...
    private int getTouchedArea(float xPos, float yPos) {
        ...
        float xEnd = TimeLapseConstants.INVALID_APERTURE_VALUE + btnWidth;
        ...
        if (xPos >= TimeLapseConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= TimeLapseConstants.INVALID_APERTURE_VALUE && yPos < yUpperEnd) {
            return 0;
        }
        if (xPos >= TimeLapseConstants.INVALID_APERTURE_VALUE && xPos < xEnd && yPos >= yCenterStart && yPos < yCenterEnd) {
            return 1;
        }
        if (xPos < TimeLapseConstants.INVALID_APERTURE_VALUE || xPos >= xEnd || yPos < yLowerStart || yPos >= btnHeight) {
            return -1;
        }
        return 2;
    }
