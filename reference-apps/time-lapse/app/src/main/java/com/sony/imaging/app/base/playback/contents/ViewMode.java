...
public abstract class ViewMode {
    ...
    protected static final String[] CONTENTS_QUERY_PROJECTION = {"_id", "_data", "dcf_file_number", IFolder.DCF_FOLDER_NUMBER, ILocalDate.CONTENT_CREATED_LOCAL_DATE, IUtcDate.CONTENT_CREATED_UTC_DATE, "content_created_local_date_time", "content_created_utc_date_time", "exist_jpeg", "exist_raw", "exist_mpo", "rec_order", "content_type"};
    public static final String DESC = " DESC";
    public static final String EQUAL_ARGS = "=?";
    protected static final String MSG_CANCEL_INITIAL_QUERY = "cancelling init thread";
    protected static final String MSG_END_INITIAL_QUERY = "InitialQueryTask end";
    protected static final String MSG_END_LOAD = "ends loadMedia";
    protected static final String MSG_END_WAIT_LOAD = "waitLoadMediaComplete ends";
    protected static final String MSG_END_WAIT_UPDATE = "waitAndUpdateDatabase ends";
    protected static final String MSG_FAILED_TO_MOVE = "failed to move : ";
    ...
    protected static final String MSG_GET_RESUME_ID = "getResumeId found : ";
    protected static final String MSG_GET_TOTAL_COUNT_GROUP_IS_NONE = "getContentsTotalCount : group count is zero";
    protected static final String MSG_GET_TOTAL_POSITION_GROUP_IS_NONE = "getContentsTotalPosition : group count is zero";
    protected static final String MSG_INITIAL_QUERY_CALLBACK = "InitialQueryTask onCallback";
    protected static final String MSG_INITIAL_QUERY_CANCELLED = "InitialQueryTask cancelled";
    protected static final String MSG_INITIAL_QUERY_STARTED = "Initial Query already started";
    protected static final String MSG_JOIN_INITIAL_QUERY_THREAD = "join init thread";
    protected static final String MSG_LAST_CONTENT_NOT_FOUND = "queryLastContent NOT null but no contents : ";
    protected static final String MSG_OBSERVER_EXISTS = "registerObserver observer already exists";
    protected static final String MSG_OBSERVER_ONCHANGE = "ContentObserver.onChange";
    protected static final String MSG_SET_CONTENT_POS = "setContentPosId :";
    protected static final String MSG_START_INITIAL_QUERY = "InitialQueryTask start";
    protected static final String MSG_START_LOAD = "starts loadMedia";
    protected static final String MSG_WAIT_LOAD_CANCELLED = "waitLoadMediaComplete returns false";
    protected static final String MSG_WAIT_UPDATE_CANCELLED = "waitAndUpdateDatabase returns false";
    ...
    protected String[] mQueryProjection = CONTENTS_QUERY_PROJECTION;
    ...
    protected ContentObserver mObserver = new ContentObserver(this.mHandler) { // from class: com.sony.imaging.app.base.playback.contents.ViewMode.1
        ...
        public void onChange(boolean selfChange) {
            ...
            if (!ViewMode.this.mIsQueryDone) {
                if (ViewMode.this.mInitThread != null) {
            ...
            ViewMode.this.notifyObserver(selfChange);
        }
    };
    ...
    public abstract boolean queryGroup();

    /* JADX INFO: Access modifiers changed from: protected */
    public abstract boolean queryInGroup();
    ...
    protected void notifyObserver(boolean selfChange) {
        if (this.mUserObserver != null) {
            this.mUserObserver.dispatchChange(selfChange);
        }
    }
    ...
    public class InitialQueryThread extends Thread {
        protected ContentsManager.QueryCallback mCallback;
        public volatile boolean mCancelled;
        private volatile boolean mWantsRetry;

        /* JADX INFO: Access modifiers changed from: protected */
        public InitialQueryThread(ContentsManager.QueryCallback callback) {
    ...
    protected InitialQueryThread getInitialQueryThread(ContentsManager.QueryCallback callback) {
    ...
    public boolean startInitialQuery(final ContentsManager.QueryCallback callback, boolean forceQuery) {
        if (isInitialQueryDone() && !forceQuery) {
            ...
            this.mIsQueryDone = false;
            this.mInitThread = getInitialQueryThread(callback);
    ...
    public synchronized boolean requeryData() {
        return requeryData(false);
    }

    public synchronized boolean requeryData(boolean doQuery) {
        boolean result;
        result = false;
        if (this.mGroupCursor != null) {
            if (doQuery) {
        ...
        synchronized (this) {
            if (this.mGroupCursor != null && this.mContentsCursor != null) {
                if (this.mGroupCursor.getCount() == 0) {
                    ...
                    if (-1 == position || position >= this.mGroupCursor.getCount()) {
                        Log.e(TAG, "group position invalid");
                    }
                    int countOfOneBefore = this.mGroupCursor.getInt(this.mGroupCursor.getColumnIndexOrThrow(getColumnGroupCountOfOneBefore()));
    ...
    public synchronized int getContentsCountOfGroup(int groupId) {
        int count;
        int current = this.mGroupCursor.getPosition();
        count = this.mGroupCursor.moveToPosition(groupId) ? this.mGroupCursor.getInt(this.mGroupCursor.getColumnIndexOrThrow(getColumnGroupCount())) : -1;
        this.mGroupCursor.moveToPosition(current);
        return count;
    }
    ...
    public synchronized boolean moveGroupToNextNonEmpty(boolean doLoopBack) {
        boolean result;
        result = false;
        int position = getGroupPosition();
        while (true) {
            if (!moveGroupToNext(doLoopBack)) {
                break;
            }
    ...
    public synchronized boolean moveGroupToPreviousNonEmpty(boolean doLoopBack) {
        boolean result;
        result = false;
        int position = getGroupPosition();
        while (true) {
            if (!moveGroupToPrevious(doLoopBack)) {
                break;
            }
            if (getContentsCount() > 0) {
                result = true;
    ...
    public synchronized int getInt(String tag) {
        int i = AudioVolumeController.INVALID_VALUE;
        synchronized (this) {
            if (this.mGroupCursor != null && this.mContentsCursor != null && !this.mContentsCursor.isBeforeFirst() && !this.mContentsCursor.isAfterLast()) {
    ...
    public synchronized int getIntOfGroupAt(int position, String tag) {
        int result;
        if (this.mGroupCursor == null) {
            result = AudioVolumeController.INVALID_VALUE;
        } else {
    ...
    public SearchInfo getSearchInfoFromResumeContents(SearchInfo info) {
        try {
            ...
            if (cursor != null) {
                int folderNumber = cursor.getInt(cursor.getColumnIndexOrThrow(IFolder.DCF_FOLDER_NUMBER));
