...
public class SaUtil {
    ...
    public static byte[] getSaResult(DeviceBuffer devBuf, int size) throws Exception {
        DSP dsp = DSP.createProcessor("sony-di-dsp");
        if (size == 0) {
            size = dsp.getPropertyAsInt(devBuf, "memory-size");
        }
    ...
    public static void mediaLog(String str) {
        ...
        if (mFw == null) {
            try {
                mFw = new FileWriter("/mnt/sdcard/mediaLog.txt", true);
            } catch (Exception e) {
                e.printStackTrace();
                return;
            }
