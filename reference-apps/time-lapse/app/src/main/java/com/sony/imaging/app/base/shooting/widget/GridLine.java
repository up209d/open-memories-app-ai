...
public class GridLine extends View {
    ...
    private void xline(Canvas c) {
        c.drawLine(TimeLapseConstants.INVALID_APERTURE_VALUE, TimeLapseConstants.INVALID_APERTURE_VALUE, this.width, this.height, this.mPaint);
        c.drawLine(TimeLapseConstants.INVALID_APERTURE_VALUE, this.height, this.width, TimeLapseConstants.INVALID_APERTURE_VALUE, this.mPaint);
    }
    ...
    private void separateline(int separateWidthNum, int separateHeightNum, Canvas c) {
        ...
        for (int i = 1; i < separateWidthNum; i++) {
            c.drawLine(widthInterval * i, TimeLapseConstants.INVALID_APERTURE_VALUE, widthInterval * i, this.height, this.mPaint);
        }
        for (int i2 = 1; i2 < separateHeightNum; i2++) {
            c.drawLine(TimeLapseConstants.INVALID_APERTURE_VALUE, heightInterval * i2, this.width, heightInterval * i2, this.mPaint);
        }
    }
}
