...
public class MenuAccessor {
    ...
    public void initialize(Context con) {
        ...
        this.mTopMenu = this.mMenuTable.createMenuTree(this.mContext, null);
        this.mOpening = this.mTopMenu;
        updateMainMenuList(this.mTopMenu.itemId);
    }

    @Deprecated
    void initialize(Context con, String topMenuItem, String fileName) {
        initialize(con, topMenuItem);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void initialize(Context con, String topMenuItem) {
        this.mContext = con;
        this.mMenuList.clear();
        this.mMoveHistory.clear();
        this.mTopMenu = this.mMenuTable.createMenuTree(this.mContext, topMenuItem);
    ...
    public static class DisplayMenuItem {
        ...
        private DisplayMenuItem(MenuTable.CmnViewMenuItem sub) {
            ...
            this.recMode = sub.recMode;
            this.parentItemId = sub.parent == null ? null : sub.parent.itemId;
            this.elements = sub.elements;
        }

        public DisplayMenuItem(DisplayMenuItem item) {
            this.isValid = true;
            this.isFnValid = true;
            if (item != null) {
                this.itemId = item.itemId != null ? item.itemId.toString() : null;
                this.textRes = item.textRes != null ? item.textRes.toString() : null;
                this.guideRes = item.guideRes != null ? item.guideRes.toString() : null;
                this.iconRes = item.iconRes != null ? item.iconRes.toString() : null;
                this.selectedIconRes = item.selectedIconRes != null ? item.selectedIconRes.toString() : null;
                this.layer1IconRes = item.layer1IconRes != null ? item.layer1IconRes.toString() : null;
                if (item.cautionID != null) {
                    this.cautionID = new ArrayList<>();
                    Iterator i$ = item.cautionID.iterator();
                    while (i$.hasNext()) {
                        String str = i$.next();
                        if (str != null) {
                            this.cautionID.add(str.toString());
                        }
                    }
                }
                this.optionStr = item.optionStr != null ? item.optionStr.toString() : null;
                this.fnOptionIconRes = item.fnOptionIconRes != null ? item.fnOptionIconRes.toString() : null;
                this.value = item.value != null ? item.value.toString() : null;
                this.fnActiveIcon = item.fnActiveIcon != null ? item.fnActiveIcon.toString() : null;
                if (item.recMode != null) {
                    this.recMode = new ArrayList();
                    for (String str2 : item.recMode) {
                        if (str2 != null) {
                            this.recMode.add(str2.toString());
                        }
                    }
                }
                Map<String, String> hash = item.elements;
                if (hash != null) {
                    this.elements = new HashMap();
                    for (Map.Entry<String, String> entry : hash.entrySet()) {
                        if (entry.getKey() != null || entry.getValue() != null) {
                            this.elements.put(entry.getKey().toString(), entry.getValue().toString());
                        }
                    }
                }
            }
    ...
    public ArrayList<String> getMenuList(String topMenuName) {
        if (this.mTopMenu == null) {
            this.mTopMenu = this.mMenuTable.createMenuTree(this.mContext, topMenuName);
        }
    ...
    public ArrayList<String> getSpecifiedMenuList(String itemId) {
        ...
        if (itemId != null) {
            MenuTable.CmnViewMenuItem partTreeRoot = this.mMenuTable.createMenuTree(this.mContext, itemId);
    ...
    public DisplayMenuItem getDisplayItem(String itemId) {
        ...
        if (item == null) {
            ...
            if (cvmi != null) {
                ...
                StringBuilder sb = StringBuilderThreadLocal.getScratchBuilder();
                sb.replace(0, sb.length(), NOT_FOUND_ITEMID);
                sb.append(itemId);
                Log.w(TAG, sb.toString());
                StringBuilderThreadLocal.releaseScratchBuilder(sb);
            }
    ...
    public DisplayMenuItem getCopyDisplayItem(String itemId) {
        DisplayMenuItem item = getDisplayItem(itemId);
        return new DisplayMenuItem(item);
    }

    public void creteHistory(String itemId) {
        this.mMoveHistory.clear();
        List<MenuTable.CmnViewMenuItem> foundItems = this.mMenuTable.findMenuItemList(itemId);
        if (foundItems != null) {
            for (int i = foundItems.size() - 1; i > 0; i--) {
                MenuTable.CmnViewMenuItem cvmi = foundItems.get(i);
                pushMenuHistory(new HistoryItem(cvmi.itemId, cvmi.nextMenuID));
            }
        }
    }
}
