...
public class LastBastionMenuLayout extends BaseMenuLayout {
    ...
    public void onResume() {
        AppLog.enter(TAG, AppLog.getMethodName());
        this.mEmc = BMExposureModeController.getInstance();
        super.onResume();
        displayCaution();
        AppLog.exit(TAG, AppLog.getMethodName());
    }
    ...
    public void onPause() {
        AppLog.enter(TAG, AppLog.getMethodName());
        CautionUtilityClass.getInstance().disapperTrigger(this.mEmc.getCautionId());
        CautionUtilityClass.getInstance().setDispatchKeyEvent(this.mEmc.getCautionId(), null);
        super.onPause();
        AppLog.exit(TAG, AppLog.getMethodName());
    }
    ...
    protected void displayCaution() {
        AppLog.enter(TAG, AppLog.getMethodName());
        IkeyDispatchEach mKey = new IkeyDispatchEach() { // from class: com.sony.imaging.app.bracketpro.menu.layout.LastBastionMenuLayout.1
        ...
        CautionUtilityClass.getInstance().requestTrigger(this.mEmc.getCautionId());
        CautionUtilityClass.getInstance().setDispatchKeyEvent(this.mEmc.getCautionId(), mKey);
        AppLog.exit(TAG, AppLog.getMethodName());
    }
}
