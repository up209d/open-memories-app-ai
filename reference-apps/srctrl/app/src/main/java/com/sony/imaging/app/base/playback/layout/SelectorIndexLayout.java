...
public abstract class SelectorIndexLayout extends IndexLayoutGridViewBase implements NotificationListener {
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
        ...
        if (-2 != arg2) {
            if (onContentEntered(arg2)) {
                beep = getEditService().isChecked(arg2) ? "BEEP_ID_ON" : "BEEP_ID_OFF";
            }
        } else if (onGroupEntered()) {
            beep = getEditService().isGroupChecked() ? "BEEP_ID_ON" : "BEEP_ID_OFF";
        }
    ...
    protected boolean onContentEntered(int position, int[] result) {
        ...
        Log.d(this.TAG, LogHelper.getScratchBuilder(MSG_SELECT).append(position).append(" : ").append(result[0]).toString());
        invalidate();
        return result[0] == 0;
    }
