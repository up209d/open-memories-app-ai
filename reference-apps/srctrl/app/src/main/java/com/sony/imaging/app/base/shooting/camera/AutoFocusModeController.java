...
public class AutoFocusModeController extends AbstractController {
    ...
    public String convertApp2PF(String focusMode) {
        ...
        builder.replace(0, builder.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_FOCUSMODE).append(focusMode);
    ...
    public void setValue(String value) {
        ...
        StringBuilder builder = sStringBuilder.get();
        builder.replace(0, builder.length(), LOG_MSG_SETAUTOFOCUSMODE).append(mode);
    ...
    public String getValue() {
        ...
        StringBuilder builder = sStringBuilder.get();
        builder.replace(0, builder.length(), LOG_MSG_GETAUTOFOCUSMODE).append(value);
        Log.i(TAG, builder.toString());
    ...
    public List<String> getSupportedValue(int mode) {
        ...
        StringBuilder builder = sStringBuilder.get();
        builder.replace(0, builder.length(), LOG_MSG_GETSUPPORTEDAUTOFOCUSMODES).append(supportValues);
        Log.i(TAG, builder.toString());
    ...
    public List<String> getAvailableValue() {
        ...
        if (supporteds != null) {
            ...
            for (String mode : supporteds) {
                if (AvailableInfo.isAvailable("setFocusMode", "auto", API_NAME, convertApp2PF(mode))) {
                    ret.add(mode);
                }
            }
        }
    ...
    public boolean isAvailable(String tag) {
        List<String> list;
        if (!tag.equals("AutoFocusMode") || (list = getAvailableValue()) == null || list.size() <= 1 || !AvailableInfo.isAvailable("setFocusMode", null, API_NAME, null)) {
            return false;
        }
        return true;
    }
