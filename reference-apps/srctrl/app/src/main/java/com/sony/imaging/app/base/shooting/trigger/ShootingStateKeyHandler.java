...
public class ShootingStateKeyHandler extends ShootingKeyHandlerBase {
    ...
    public int releasedS2Key() {
        if (4 == ExecutorCreator.getInstance().getRecordingMode() || isBulbOpenedBy(1)) {
            return -1;
        }
    ...
    public int pushedMovieRecKey() {
        if (Environment.isNewBizDeviceActionCam() && (1 == ExecutorCreator.getInstance().getRecordingMode() || 4 == ExecutorCreator.getInstance().getRecordingMode())) {
            return pushedS2Key();
        }
        if (Environment.isMovieAPISupported() && ((BaseApp) this.target.getActivity()).isMovieModeSupported()) {
            ...
            if (Settings.getMovieButtonMode() == 0) {
                ...
                if (!writing) {
                    ...
                    state.replaceChildState("MovieCapture", data);
                    return 1;
                }
    ...
    public int releasedMovieRecKey() {
        return Environment.isNewBizDeviceActionCam() ? releasedS2Key() : super.releasedMovieRecKey();
    }
}
