...
public class BaseMenuLayout extends Layout implements MenuStateClosedListener {
    ...
    public static final String BUNDLE_KEY_SELECTED_ITEM = "selectedItem";
    ...
    protected String getMenuLayoutID() {
        return "ID_" + getClass().getName().substring(getClass().getName().lastIndexOf(StringBuilderThreadLocal.PERIOD) + 1).toUpperCase(Locale.US);
    }
    ...
    public void onResume() {
        ...
        if (Environment.isNewBizDeviceActionCam()) {
            setKeyBeepPattern(getBeepPattern());
        }
    ...
    public int pushedPlayBackKey() {
        ...
        if (Environment.isMovieAPISupported()) {
            boolean isMovieRecmode = 2 == ExecutorCreator.getInstance().getRecordingMode() || 8 == ExecutorCreator.getInstance().getRecordingMode();
            isMovieRecording = isMovieRecmode && MovieShootingExecutor.isMovieRecording();
        }
        boolean isIntervalRecRecording = false;
        if (ExecutorCreator.isIntervalRecEnable() && 4 == ExecutorCreator.getInstance().getRecordingMode() && 200 != IntervalRecExecutor.getIntervalRecStatus()) {
            isIntervalRecRecording = true;
        }
        if (isMovieRecording || isIntervalRecRecording) {
            return 1;
        }
        closeMenuLayout(null);
        return 0;
    ...
    protected void openMenuLayout(String menulayoutid, Bundle bundle) {
    ...
    public void openNextMenu(String itemId, String layoutId, boolean history, Bundle bundle) {
        ...
        if ((!myLayoutId.equals(layoutId) || !myItemId.equals(itemId)) && itemId != null && itemId.length() > 0) {
            ...
            passedParcelable.setMenuData(itemId, myLayoutId, passedService.getMenuItemExecType(itemId), layoutId, passedService);
    ...
    public void openPreviousMenu() {
        ...
        this.data.putString(BUNDLE_KEY_SELECTED_ITEM, service.getMenuItemId());
        ...
        parcelable.setMenuData("back", myLayoutId, service.getMenuItemExecType(item.itemId), nextLayoutId, service);
    ...
    public void openRootMenu() {
        ...
        parcelable.setMenuData(itemId, getMenuLayoutID(), service.getMenuItemExecType(itemId), layoutId, service);
        this.data.putParcelable(MenuDataParcelable.KEY, parcelable);
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public View getFunctionGuideView() {
        ...
        if (nextItemId != null) {
            ...
            if (keyFunction.isValid()) {
                if (!isMovieRecording || !CustomizableFunction.ExposureMode.equals(keyFunction)) {
                    if (!isIntervalRecRecording || !CustomizableFunction.ExposureMode.equals(keyFunction)) {
            ...
            if (!isMovieRecording && !isIntervalRecRecording && (cautionId = service.getMenuItemCautionId(nextItemId)) != null) {
