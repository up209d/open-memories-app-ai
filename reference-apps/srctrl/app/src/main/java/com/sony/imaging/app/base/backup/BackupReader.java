...
public class BackupReader {
    ...
    public static IndexPlaybackMode getIndexPlaybackMode() {
        if (IndexPlaybackMode.INVALID == mIndexPlaybackmode) {
            ...
            Log.i(TAG, builder.replace(0, builder.length(), MSG_PREFIX_INDEX_NUM).append(num).toString());
    ...
    public static RecordedDateMode getRecordedDateMode() {
        if (RecordedDateMode.INVALID == mRecordedDateMode) {
            ...
            StringBuilder builder = StringBuilderThreadLocal.getScratchBuilder();
            Log.i(TAG, builder.replace(0, builder.length(), MSG_PREFIX_DATE_FORMAT).append(dateFormat).toString());
            StringBuilderThreadLocal.releaseScratchBuilder(builder);
            switch (dateFormat) {
                case 0:
    ...
    public static VPicDisplay getVPicDisplay() {
        if (VPicDisplay.INVALID == mVPicDisplay) {
            ...
            StringBuilder builder = StringBuilderThreadLocal.getScratchBuilder();
            Log.i(TAG, builder.replace(0, builder.length(), MSG_PREFIX_VPICDISPLAY).append(autoRotate).toString());
            StringBuilderThreadLocal.releaseScratchBuilder(builder);
    ...
    public static WideDisplay getWideDisplay() {
        if (WideDisplay.INVALID == mWideDisplay) {
            ...
            StringBuilder builder = StringBuilderThreadLocal.getScratchBuilder();
            Log.i(TAG, builder.replace(0, builder.length(), MSG_PREFIX_WIDEDISPLAY).append(wideImage).toString());
            StringBuilderThreadLocal.releaseScratchBuilder(builder);
    ...
    public static ConfirmationCusorPosition getDeleteConfirmationDisplay() {
        ...
        if (ConfirmationCusorPosition.INVALID == mDeleteConfirmationCursor) {
            ...
            StringBuilder builder = StringBuilderThreadLocal.getScratchBuilder();
            Log.i(TAG, builder.replace(0, builder.length(), MSG_PREFIX_DELETE_CONFIRMATION).append(cursorPosition).toString());
            StringBuilderThreadLocal.releaseScratchBuilder(builder);
