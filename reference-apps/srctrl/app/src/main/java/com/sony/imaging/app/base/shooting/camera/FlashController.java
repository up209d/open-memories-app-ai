...
public class FlashController extends ShootingModeController implements CompensationControllable {
    ...
    public boolean isAvailable(String tag) {
        if (FLASHMODE.equals(tag)) {
            ...
            boolean ret = AvailableInfo.isAvailable("setFlashMode", null);
            if (!ret) {
                return AvailableInfo.isAvailable("setFlashMode", null, API_NAME_TYPE, null);
            }
            return ret;
        }
    ...
    protected void removeUnavailableValue(List<String> availables) {
        ...
        if (ExposureModeController.PROGRAM_AUTO_MODE.equals(expMode) || "Aperture".equals(expMode) || "Shutter".equals(expMode) || "Manual".equals(expMode)) {
    ...
    protected void setDefaultValueEx(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        if (ExposureModeController.PROGRAM_AUTO_MODE.equals(expMode) || "Aperture".equals(expMode) || "Shutter".equals(expMode) || "Manual".equals(expMode)) {
            ...
            if ("off".equals(flashMode)) {
                ...
                if (!AvailableInfo.isAvailable("setFlashMode", "on", API_NAME_TYPE, flashType)) {
                    if (AvailableInfo.isAvailable("setFlashMode", "on", API_NAME_TYPE, FRONTSYNC)) {
                        flashType = FRONTSYNC;
                    } else {
                        flashType = null;
                    }
            ...
            if (supporteds2 != null) {
                for (String mode : supporteds2) {
                    if ("off".equals(mode) || "auto".equals(mode)) {
                        if (AvailableInfo.isAvailable("setFlashMode", mode)) {
                            availables.add(mode);
                        }
                    } else if (AvailableInfo.isAvailable("setFlashMode", "on", API_NAME_TYPE, mode)) {
                        availables.add(mode);
                    }
                }
    ...
    public boolean isUnavailableSceneFactor(String tag) {
        if (FLASHMODE.equals(tag)) {
            boolean mode = isUnavailableAPISceneFactor("setFlashMode", null);
            return mode;
        }
