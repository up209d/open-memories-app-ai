...
public class AuthUtil {
    ...
    public static String generateHashCode(String str, String key) {
        try {
            SecretKeySpec secretKeySpec = new SecretKeySpec(key.getBytes("UTF-8"), AuthConfig.ALGORISM);
    ...
    public static String toHexString(byte[] bytes) {
        ...
        for (byte b : bytes) {
            String b2 = Integer.toHexString(b & LiveviewCommon.COMMON_HEADER_START_BYTE);
            if (b2.length() == 1) {
