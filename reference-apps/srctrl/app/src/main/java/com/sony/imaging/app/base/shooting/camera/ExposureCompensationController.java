...
public class ExposureCompensationController extends AbstractController implements CompensationControllable {
    ...
    public List<String> getSupportedValue(String tag) {
        ...
        if ("ExposureCompensation".equals(tag)) {
            ...
            if (((Integer) range.first).intValue() <= 9 && ((Integer) range.second).intValue() >= EV_RANGE_PULS_MINUS_3_MIN && getExposureCompensationStep() != 0.5f) {
                return null;
            }
    ...
    public List<String> getAvailableValue(String tag) {
        ...
        if (supporteds != null) {
            for (String mode : supporteds) {
                if (AvailableInfo.isAvailable("setExposureCompensation", mode)) {
                    availables.add(mode);
                }
            }
        }
    ...
    public boolean isUnavailableSceneFactor(String tag) {
        boolean b = isUnavailableAPISceneFactor("setExposureCompensation", null);
        return b;
    }
    ...
    public boolean isExposureCompensationAvailable() {
        ...
        if (getSupportedValue(null) != null) {
            if (CameraSetting.getPfApiVersion() >= 2) {
                ret = AvailableInfo.isAvailable("setExposureCompensation", null);
            } else {
