...
public class DevelopmentState extends StateBase {
    ...
    public void notifyProgressStatusForBulb() {
        notifyProgressStatus(0.0d);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public void notifyProgressStatus(double progress) {
        ...
        if (!isInvalidSetting()) {
            ...
            if (this.mProgressTimerTask == null) {
                this.mProgressTimerTask = new TimerTask() { // from class: com.sony.imaging.app.base.shooting.DevelopmentState.1
                    ...
                    public void run() {
                        DevelopmentState.this.notifyProgressStatus(0.0d);
                    }
                };
            }
            Pair<Integer, Integer> ss = CameraSetting.getInstance().getShutterSpeed();
            if (ss != null) {
                shutterSpeed = (((Integer) ss.first).intValue() * SsdpDevice.RETRY_INTERVAL) / ((Integer) ss.second).intValue();
            } else {
    ...
    public boolean handleMessage(Message msg) {
        ...
        setNextState("Capture", returnBundle);
        return true;
    }
