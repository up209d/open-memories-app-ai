    ...
    */
    ...
    private static Mode decodeAsciiSegment(BitSource bits, StringBuilder result, StringBuilder resultTrailer) throws FormatException {
        ...
        do {
            ...
            if (oneByte <= 128) {
                if (upperShift) {
                    oneByte += LiveviewCommon.PAYLOAD_HEADER_SIZE;
                }
        ...
        while (bits.available() != 8 && (firstByte = bits.readBits(8)) != 254) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                switch (shift) {
                    ...
                    case 1:
                        if (upperShift) {
                            result.append((char) (cValue + LiveviewCommon.PAYLOAD_HEADER_SIZE));
                            upperShift = false;
                        } else {
    ...
    private static int unrandomize255State(int randomizedBase256Codeword, int base256CodewordPosition) {
        int pseudoRandomNumber = ((base256CodewordPosition * 149) % BatteryIcon.BATTERY_STATUS_CHARGING) + 1;
