...
public final class Encoder {
    ...
    private static boolean isOnlyDoubleByteKanji(String content) {
        try {
            ...
            for (int i = 0; i < length; i += 2) {
                int byte1 = bytes[i] & BatteryIcon.BATTERY_STATUS_CHARGING;
    ...
    static byte[] generateECBytes(byte[] dataBytes, int numEcBytesInBlock) {
        ...
        for (int i = 0; i < numDataBytes; i++) {
            toEncode[i] = dataBytes[i] & LiveviewCommon.COMMON_HEADER_START_BYTE;
        }
    ...
    static void appendKanjiBytes(String content, BitArray bits) throws WriterException {
        try {
            ...
            int length = bytes.length;
            for (int i = 0; i < length; i += 2) {
                int byte1 = bytes[i] & LiveviewCommon.COMMON_HEADER_START_BYTE;
                int byte2 = bytes[i + 1] & LiveviewCommon.COMMON_HEADER_START_BYTE;
                ...
                int encoded = ((subtracted >> 8) * 192) + (subtracted & BatteryIcon.BATTERY_STATUS_CHARGING);
