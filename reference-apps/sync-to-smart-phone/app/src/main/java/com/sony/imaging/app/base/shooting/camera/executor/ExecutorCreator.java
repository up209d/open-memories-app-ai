...
public abstract class ExecutorCreator extends HandlerThread {
    ...
    private static final int REC_MODE_CAMERA_MOVIES = 2;
    ...
    protected boolean isCameraModeStill() {
        return (getRecordingMode() & 5) != 0;
    }

    protected boolean isCameraModeMovie() {
        return (getRecordingMode() & 2) != 0;
    }
    ...
    protected BaseShootingExecutor setup(BaseShootingExecutor executor, IAdapter adapter, IProcess process) {
        ...
        builder.replace(0, builder.length(), LOG_CAMERA).append(LOG_SET_UP).append(" ").append(LOG_EXECUTOR).append(executor).append(": ").append(LOG_ADAPTER).append(adapter).append(": ").append(LOG_PROCESS).append(process).append(": ");
