...
public class AELController extends AbstractController {
    ...
    public boolean getAELockButtonState() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETAUTOEXPOSURELOCK).append(ael);
    ...
    public void holdAELock(boolean control) {
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_HOLDAELOCK).append(control);
    ...
    private void setValue(String onoff) {
        ...
        if (list != null && list.contains(onoff)) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETAUTOEXPOSURELOCK).append(onoff);
            Log.i(TAG, STRBUILD.toString());
