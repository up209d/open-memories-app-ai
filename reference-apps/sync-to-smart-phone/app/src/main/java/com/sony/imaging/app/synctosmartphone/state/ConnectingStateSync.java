...
public class ConnectingStateSync extends State {
    private static final String CONNECTING_LAYOUT = "ConnectingLayoutSync";
    private int bootMode;
    private Timer timer = null;
    private static final String TAG = ConnectingStateSync.class.getSimpleName();
    private static ConnectingStateSync mInstance = null;
    private static boolean mConnectinglayout_status = false;
    ...
    public void onResume() {
        ...
        Log.v(TAG, "Bundle data :  = this.data");
        this.bootMode = data.getInt(ConstantsSync.BOOT_MODE);
        Log.v(TAG, "data.getInt : bootMode = bootMode");
        openLayout("ConnectingLayoutSync", data);
        Log.v(TAG, "openLayout : CONNECTING_LAYOUT = ");
        NetworkStateUtil.getInstance().setState(this, data);
        setConnectingLayoutStatus(true);
        if (this.bootMode == 1) {
    ...
    private class cancelTimerTask extends TimerTask {
        ...
        public void run() {
            Log.i(ConnectingStateSync.TAG, "cancelTimerTask");
            ConnectingStateSync.this.invokeFinishProcess();
            Bundle data = new Bundle();
            data.putInt(ConstantsSync.BOOT_MODE, ConnectingStateSync.this.bootMode);
            if (ConnectingStateSync.this.bootMode == 0) {
            ...
            ConnectingStateSync.this.setNextState(ConstantsSync.DIALOG_STATE, data);
        }
    }
    ...
    public void onPause() {
        ...
        setConnectingLayoutStatus(false);
        closeLayout("ConnectingLayoutSync");
        super.onPause();
    }

    public static ConnectingStateSync getInstance() {
        if (mInstance == null) {
            mInstance = new ConnectingStateSync();
        }
        return mInstance;
    }

    public void setConnectingLayoutStatus(boolean value) {
        mConnectinglayout_status = value;
        Log.v(TAG, "setConnectingLayoutStatus = " + value);
    }

    public boolean getConnectingLayoutStatus() {
        Log.v(TAG, "getConnectingLayoutStatus = " + mConnectinglayout_status);
        return mConnectinglayout_status;
    }
