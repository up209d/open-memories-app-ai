...
public class MovieRecState extends MovieStateBase implements MediaRecorder.OnRecordListener, IUserChanging {
    ...
    public void onStopped(MediaRecorder mr) {
        boolean isRecDisabledOnStreamWriting;
        ...
        if (6 <= Environment.getVersionPfAPI()) {
            isRecDisabledOnStreamWriting = 1 != ScalarProperties.getInt("ui.movie.rec.quick.restart.supported");
        } else {
            isRecDisabledOnStreamWriting = 2 == category || 2 == category;
        }
        ...
        if (true == intelligentActive || true == isRecDisabledOnStreamWriting) {
