...
public class DatabaseIcon extends ImageView implements AbstractRelativeLayoutGroup.IVisibilityChange {
    ...
    public void refreshIcon() {
        Drawable d;
        int dbState = 0;
        if (ExecutorCreator.getInstance().getRecordingMode() == 2 || ExecutorCreator.getInstance().getRecordingMode() == 8) {
            String movieFormat = MovieFormatController.getInstance().getValue("movie_format");
