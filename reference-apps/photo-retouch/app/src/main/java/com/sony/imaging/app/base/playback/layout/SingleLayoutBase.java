...
public abstract class SingleLayoutBase extends OptimizedImageLayoutBase implements IModableLayout, TouchArea.OnTouchAreaListener {
    ...
    private static final Rect[] YUV_LAYOUT_TABLE_HISTOGRAM = {new Rect(64, 48, 348, 294), new Rect(104, 72, 392, PhotoRetouchKikiLog.PHOTO_RETOUCH_KIKILOG_APP_START), new Rect(140, 72, 356, PhotoRetouchKikiLog.PHOTO_RETOUCH_KIKILOG_APP_START)};
    ...
    public void updateOptimizedImageLayoutParam(OptimizedImageView imgView, ContentsManager mgr) {
        ...
        if (targetLayout != null) {
            ...
            if (!targetLayout.equals(mScratchRect)) {
                ...
                Log.i(this.TAG, LogHelper.getScratchBuilder(MSG_SET_PIVOT).append(p.x).append(LogHelper.MSG_COMMA).append(p.y).toString());
                imgView.setPivot(p);
