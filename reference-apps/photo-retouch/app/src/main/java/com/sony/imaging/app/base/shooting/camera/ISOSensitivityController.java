...
public class ISOSensitivityController extends AbstractController {
    ...
    protected static void createSupportedValueByBase() {
        ...
        mSupportedISOByBase.add(ISO_100);
        ...
        mSupportedISO_ExpandedByBase.add(Integer.toString(80));
    ...
    public void setValue(String itemId, String value) {
        ...
        try {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_ILLEGALVALUE).append(value);
    ...
    private void setIsoValue(String value) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETISOSENSITIVITY).append(raw);
    ...
    private String getIsoValue() {
        ...
        int raw = ((CameraEx.ParametersModifier) params.second).getISOSensitivity();
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETISOSENSITIVITY).append(raw);
        Log.i(TAG, STRBUILD.toString());
    ...
    private static List<String> createSupportedValueArray(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDISOSENSITIVITIES).append(supported);
        Log.i(TAG, STRBUILD.toString());
        if (supported != null) {
    ...
    private static HashMap<String, String> createSupportedExpandedIso(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        if (CameraSetting.getPfApiVersion() >= 2) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDEXPANDEDISOSENSITIVITIES).append(supported);
            Log.i(TAG, STRBUILD.toString());
            if (supported != null) {
                list = new HashMap<>();
                for (Integer i : supported) {
    ...
    public void increment() {
        ...
        if (nowIndex < 0) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_ILLEGALVALUE).append(nowValue);
            Log.e(TAG, STRBUILD.toString());
        } else {
    ...
    public void onGetInitParameters(Pair<Camera.Parameters, CameraEx.ParametersModifier> params) {
        ...
        String value = getIsoValue();
        if ((mISO_Expanded != null && mISO_Expanded.containsKey(value) && mSupportedISO_ExpandedByBase != null && !mSupportedISO_ExpandedByBase.contains(value)) || (mSupportedISOSensitivities != null && mSupportedISOSensitivities.contains(value) && mSupportedISOByBase != null && !mSupportedISOByBase.contains(value))) {
