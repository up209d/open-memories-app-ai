...
public class AVIndexProvider extends ProviderHelper {
    ...
    public static class OptImgCache {
        ...
        protected void close() {
            if (this.mImg != null) {
                Log.i(AVIndexProvider.TAG, LogHelper.getScratchBuilder(AVIndexProvider.MSG_RELEASE_IMG_CACHE).append(this.mID).append(LogHelper.MSG_COMMA).append(this.mType).toString());
                this.mImg.release();
            }
            this.mImg = null;
            this.mID = null;
    ...
    public OptimizedImage getOptimizedImage(ContentsIdentifier id, int imageType, ContentInfo[] info) {
        ...
        if (this.mOptImgCache.hasData(id.data, imageType)) {
            Log.d(TAG, LogHelper.getScratchBuilder(MSG_GET_IMG_FROM_CACHE).append(id.data).append(LogHelper.MSG_COMMA).append(imageType).toString());
        } else {
