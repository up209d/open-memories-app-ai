...
public class CameraNotificationManager extends NotificationManager {
    ...
    public void requestSyncNotify(String tag, Object value) {
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_REQUESTSYNCNOTIFY).append(LOG_MSG_TAG).append(tag).append(", ").append(LOG_MSG_VALUE).append(value);
        ...
        if (isValueChanged) {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_NOTCHANGED).append(LOG_MSG_TAG).append(tag);
