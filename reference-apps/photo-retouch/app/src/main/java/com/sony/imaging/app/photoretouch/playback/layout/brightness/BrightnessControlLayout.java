...
public class BrightnessControlLayout extends EVFOffLayout implements SeekBar.OnSeekBarChangeListener {
    private SeekBar mSliderBar;
    private final String TAG = BrightnessControlLayout.class.getSimpleName();
    private OptimizedImageView mOptimizedImageView = null;
    private LCEControl mLceControl = null;
    private BCSImageAdapter mAdapter = null;
    private PhotoRetouchSubMenuLayout mPhotoRetouchSubMenuLayout = null;
    int[] arr = {5, 51, 96, 143, 188, 234, 279, 325, 369, 416, 461, 506, AppRoot.USER_KEYCODE.IR_SHUTTER_2SEC};
    OptimizedImage mEditImage = null;

    @Override // com.sony.imaging.app.fw.Layout, com.sony.imaging.app.fw.FakeFragment
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        super.onCreateView(inflater, container, savedInstanceState);
        View currentView = obtainViewFromPool(R.layout.brightness_control);
        ...
        ImageView backGround = (ImageView) currentView.findViewById(R.id.brightness_background);
        ...
        this.mSliderBar.setOnTouchListener(new View.OnTouchListener() { // from class: com.sony.imaging.app.photoretouch.playback.layout.brightness.BrightnessControlLayout.1
    ...
    public void onProgressChanged(SeekBar bar, int progress, boolean arg2) {
        ...
        if (this.mAdapter.isStable()) {
            setCursorOnScales(progress);
            shuffleGalleryItems(progress);
        }
    }

    private void shuffleGalleryItems(int progress) {
        Log.d("INSIDE", "shuffleGalleryItems");
        Log.d("progress", "" + progress);
        if (progress == 0) {
            progress = 5;
        } else if (progress > 553) {
            progress = AppRoot.USER_KEYCODE.IR_SHUTTER_2SEC;
        }
        int index = 0;
        ...
        while (true) {
            if (i < len - 1) {
                if (progress < this.arr[i] || progress >= this.arr[i + 1]) {
                    i++;
                } else {
                    Log.d("INSIDE", "is found");
                    index = i;
                    isFound = true;
                    break;
                }
        ...
        if (!isFound) {
            Log.d("INSIDE", "is not found");
            index = len - 1;
        }
        Log.d("index", "" + index);
        this.mAdapter.setSelection(index);
    }
    ...
    public void setCursorOnScales(int progress) {
        if (progress < this.arr[0]) {
            this.mSliderBar.setProgress(this.arr[0]);
            return;
        }
        if (progress >= this.arr[0] && progress < this.arr[1]) {
            this.mSliderBar.setProgress(this.arr[0]);
            return;
        }
        if (progress >= this.arr[1] && progress < this.arr[2]) {
            this.mSliderBar.setProgress(this.arr[1]);
            return;
        }
        if (progress >= this.arr[2] && progress < this.arr[3]) {
            this.mSliderBar.setProgress(this.arr[2]);
            return;
        }
        if (progress >= this.arr[3] && progress < this.arr[4]) {
            this.mSliderBar.setProgress(this.arr[3]);
            return;
        }
        if (progress >= this.arr[4] && progress < this.arr[5]) {
            this.mSliderBar.setProgress(this.arr[4]);
            return;
        }
        if (progress >= this.arr[5] && progress < this.arr[6]) {
            this.mSliderBar.setProgress(this.arr[5]);
            return;
        }
        if (progress >= this.arr[6] && progress < this.arr[7]) {
            this.mSliderBar.setProgress(this.arr[6]);
            return;
        }
        if (progress >= this.arr[7] && progress < this.arr[8]) {
            this.mSliderBar.setProgress(this.arr[7]);
            return;
        }
        if (progress >= this.arr[8] && progress < this.arr[9]) {
            this.mSliderBar.setProgress(this.arr[8]);
            return;
        }
        if (progress >= this.arr[9] && progress < this.arr[10]) {
            this.mSliderBar.setProgress(this.arr[9]);
            return;
        }
        if (progress >= this.arr[10] && progress < this.arr[11]) {
            this.mSliderBar.setProgress(this.arr[10]);
            return;
        }
        if (progress >= this.arr[11] && progress < this.arr[12]) {
            this.mSliderBar.setProgress(this.arr[11]);
        } else if (progress >= this.arr[12]) {
            this.mSliderBar.setProgress(this.arr[12]);
        }
    }
    ...
    public void onResume() {
        ...
        this.mLceControl.setContrast(3);
        this.mLceControl.setSaturation(3);
        this.mAdapter = new BCSImageAdapter(this.mLceControl, 13, this.mOptimizedImageView, new BCSImageAdapter.ChangeParameter() { // from class: com.sony.imaging.app.photoretouch.playback.layout.brightness.BrightnessControlLayout.2
            ...
            public void setParameter(int position, LCEControl lce) {
                lce.setBrightness(position);
            }
        });
        this.mAdapter.setSelection(6);
        changeSliderProgress(6);
        setKeyBeepPattern(0);
    }
    ...
    private void editImage() {
        ImageEditor.releaseOptImage(this.mLceControl.getScaledImage());
        if (this.mAdapter.getSelection() != 6) {
    ...
    public int onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode2) {
            ...
            case AppRoot.USER_KEYCODE.CENTER /* 232 */:
                if (PhotoRetouchSubMenuLayout.isRepeat(event) || (ImageEditor.runnableCount > 0 && this.mAdapter.getSelection() != 6)) {
                    return -1;
                }
                this.mAdapter.terminate();
                editImage();
