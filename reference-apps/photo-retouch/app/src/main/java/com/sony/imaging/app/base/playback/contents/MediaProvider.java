...
public class MediaProvider extends ProviderHelper {
    ...
    public int getInt(Cursor c, String tag) {
        if (!tag.equals("dcf_folder_number")) {
    ...
    public Bitmap getJpeg(ContentsIdentifier id) {
        ...
        Log.i(TAG, "getJpeg org size: " + options.outWidth + LogHelper.MSG_COMMA + options.outHeight);
        int sampling = Math.max(options.outWidth / IMG_TARGET_WIDTH, options.outHeight / IMG_TARGET_HEIGHT);
        options.inJustDecodeBounds = false;
        options.inSampleSize = sampling;
        Bitmap bitmap = BitmapFactory.decodeFile(id.data, options);
        Log.i(TAG, "getJpeg scaled size: " + options.outWidth + LogHelper.MSG_COMMA + options.outHeight);
        return bitmap;
    }
