...
public class ScalarTimeZone extends TimeZone {
    ...
    private ScalarTimeZone(PlainTimeZone pZone) {
        this(null, pZone.gmtDiff * 60 * Constant.TRANSITION_INDEXPB, pZone.summerTimeDiff * 60 * Constant.TRANSITION_INDEXPB);
    }
    ...
    private static String createID(int rawOffset) {
        int rawOffset2 = rawOffset / 60000;
    ...
    public static void dumpAvailableIDs() {
        ...
        for (int i = 0; i < length; i++) {
            serialized = serialized + i + " -> " + zones[i] + LogHelper.MSG_COLON;
            if (i % 10 == 9) {
