...
public abstract class ShootingExecutor extends BaseShootingExecutor {
    ...
    public static class MyShutterCb implements CameraEx.ShutterListener {
        ...
        public void onShutter(int status, CameraEx cameraEx) {
            ...
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append(ShootingExecutor.LOG_ON_SHUTTER).append(": status ").append(status);
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    private static class MyPictureReviewStartCb implements CameraEx.PictureReviewStartListener {
        ...
        public void onStart(CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append(ShootingExecutor.LOG_ON_PICTURE_REVIEW_START);
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    private static class MyPictureReviewInfoCb implements CameraEx.PictureReviewInfoListener {
        ...
        public void onGetInfo(CameraEx.ReviewInfo info, CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append(ShootingExecutor.LOG_ON_GET_PICTURE_REVIEW_INFO);
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
            if (ShootingExecutor.sQueue != null) {
    ...
    static class MyJpegCb implements CameraEx.JpegListener {
        ...
        public void onPictureTaken(byte[] jpeg, CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append(ShootingExecutor.LOG_ON_PICTURE_TAKEN);
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    protected static class MyShootingPreviewStartCb extends BaseShootingExecutor.MyBasePreviewStartCb {
        protected MyShootingPreviewStartCb() {
        }

        @Override // com.sony.imaging.app.base.shooting.camera.executor.BaseShootingExecutor.MyBasePreviewStartCb
        public void onStart(CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append("onPreviewStart");
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
            if (ShootingExecutor.sQueue != null) {
    ...
    public static class StartPictureReviewRunnable implements Runnable {
        ...
        public void run() {
            BaseShootingExecutor.STRBUILD_UI.replace(0, BaseShootingExecutor.STRBUILD_UI.length(), "UI :").append(ShootingExecutor.LOG_START_PICTURE_REVIEW).append("Runnable");
            Log.d(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_UI.toString());
    ...
    public void onStartShutter(int status, CameraEx cameraEx) {
        ...
        STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), "CAMERA :").append(LOG_ON_START_SHUTTER).append(": status ").append(status);
    ...
    public static class StartShutterRunnable implements Runnable {
        ...
        @Override // java.lang.Runnable
        public void run() {
            BaseShootingExecutor.STRBUILD_UI.replace(0, BaseShootingExecutor.STRBUILD_UI.length(), "UI :").append(ShootingExecutor.LOG_ON_SHUTTER).append("Runnable").append(": status ").append(this.mStatus);
            Log.d(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_UI.toString());
    ...
    private static class CancelTakePictureRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (!ShootingExecutor.isLockCancelTakePicture) {
                BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), ShootingExecutor.LOG_CANCEL_TAKE_PICTURE);
                Log.d(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    public void lockCancelTakePicture(boolean lock) {
        if (isLockCancelTakePicture != lock) {
            ...
            if (!lock && sIsRequestedCancel) {
                STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), LOG_CANCEL_TAKE_PICTURE).append(": status ").append("lock");
    ...
    protected static class SetRecordingMediaRunnable implements Runnable {
        ...
        @Override // java.lang.Runnable
        public void run() {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append("setRecordingMedia");
            Log.d(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    protected static class MyRecordingMediaChangeCallback implements CameraEx.RecordingMediaChangeCallback {
        ...
        public void onRecordingMediaChange(CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), "CAMERA :").append("onRecordingMediaChanged");
            Log.i(ShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
