...
public class AntiHandBlurController extends AbstractController {
    ...
    protected AntiHandBlurController() {
        setController(this);
    }
    ...
    public void setValue(String tag, String value) {
        ...
        } else if ("movie".equals(tag) && CameraSetting.getPfApiVersion() >= 2) {
            ((CameraEx.ParametersModifier) p.second).setMovieAntiHandBlurMode(value);
            CameraSetting.getInstance().setParameters(p);
        }
        STRBUILD.replace(0, STRBUILD.length(), tag).append(LOG_MSG_SETANTIHANDBLURMODE).append(value);
        Log.i("AntiHandBlur", STRBUILD.toString());
    }
    ...
    public String getValue(String tag) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), tag).append(LOG_MSG_GETANTIHANDBLURMODE).append(value);
        Log.i("AntiHandBlur", STRBUILD.toString());
        return value;
    }
    ...
    public List<String> getSupportedValue(String tag) {
        ...
        if (!"movie".equals(tag) || CameraSetting.getPfApiVersion() < 2) {
            return null;
        }
