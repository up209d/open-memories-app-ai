...
public class AvailableInfoInhMgr extends AvailableInfoImpl {
    ...
    private final StringBuilder SELCTOR = new StringBuilder();
    ...
    */
    ...
    private boolean isFactorHelper(String factorID, int bit, Object... api) {
        Cursor featuresCursor = getFeaturesCursor(api);
        Cursor factorsCursor = getFactorsCursor(featuresCursor);
        boolean ret = isFactorWithCursor(bit, factorsCursor);
    ...
    /*
        ...
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public boolean isAvailable(android.database.Cursor r5) {
        /*
            r4 = this;
            r2 = 0
            r1 = 0
    ...
    public boolean isAvailable(Object... api) {
        Cursor featuresCursor = getFeaturesCursor(api);
        Cursor factorsCursor = getFactorsCursor(featuresCursor);
        boolean ret = isAvailable(factorsCursor);
        if (featuresCursor != null) {
            featuresCursor.close();
        }
        if (factorsCursor != null) {
    ...
    /* JADX WARN: Code restructure failed: missing block: B:7:0x0014, code lost:            r6 = r1.split(com.sony.imaging.app.base.shooting.camera.parameters.AbstractSupportedChecker.SEPARATOR);        r3 = r6.length;        r2 = 0;     */
    ...
    */
    ...
    public Cursor getFeaturesCursor(Object... api) {
        ...
        Arrays.sort(pair);
        this.SELCTOR.replace(0, this.SELCTOR.length(), GET_FEATURES_INDEX);
        for (Object obj : pair) {
            this.SELCTOR.append(obj).append(MovieFormatController.Settings.SEMI_COLON);
        }
        this.SELCTOR.append("'");
        if (AvailableInfo.DEBUG) {
            Log.i(TAG, "start query: " + ((Object) this.SELCTOR));
        }
        Cursor c = this.mDB.rawQuery(this.SELCTOR.toString(), null);
        if (AvailableInfo.DEBUG) {
            Log.i(TAG, "finish");
        }
        return c;
    ...
    public Cursor getFactorsCursor(Cursor c) {
        ...
        this.SELCTOR.replace(0, this.SELCTOR.length(), GET_FACTORS);
        int featuresCount = features.capacity();
        for (int i = 0; i < featuresCount; i++) {
            this.SELCTOR.append((int) features.get(i)).append("'");
            if (i + 1 < featuresCount) {
                this.SELCTOR.append(",'");
            }
        }
        this.SELCTOR.append(LogHelper.MSG_CLOSE_BRACKET);
        if (AvailableInfo.DEBUG) {
            Log.i(TAG, "start query: " + ((Object) this.SELCTOR));
        }
        Cursor c2 = this.mDB.rawQuery(this.SELCTOR.toString(), null);
        if (AvailableInfo.DEBUG) {
            Log.i(TAG, "finish");
        }
        return c2;
    ...
    protected class Version1 implements VersionDispatcher {
        ...
        public Object[] isAvailableHelper(Camera.Parameters p, CameraEx.ParametersModifier m, MediaRecorder.Parameters mp, AudioManager.Parameters ma, boolean apiOnly, ArrayList<String> paramAPI, ArrayList<String> paramValue) {
            ...
            AvailableInfoInhMgr.this.SELCTOR.replace(0, AvailableInfoInhMgr.this.SELCTOR.length(), AvailableInfoInhMgr.GET_RELATION_VER1).append(API).append("'");
            if (AvailableInfo.DEBUG) {
                Log.i(AvailableInfoInhMgr.TAG, "start query: " + ((Object) AvailableInfoInhMgr.this.SELCTOR));
            }
            Cursor c = AvailableInfoInhMgr.this.mDB.rawQuery(AvailableInfoInhMgr.this.SELCTOR.toString(), null);
            if (AvailableInfo.DEBUG) {
                Log.i(AvailableInfoInhMgr.TAG, "finish");
            }
            int count = c.getCount();
            if (count != 0 && c.moveToFirst()) {
                mustGetAPI.clear();
                String[] getAPIs = c.getString(0).split(AbstractSupportedChecker.SEPARATOR);
                String[] originalAPI = c.getString(1).split(AbstractSupportedChecker.SEPARATOR);
                c.close();
    ...
    protected class Version2 implements VersionDispatcher {
        ...
        public Object[] isAvailableHelper(Camera.Parameters p, CameraEx.ParametersModifier parametersModifier, MediaRecorder.Parameters parameters, AudioManager.Parameters parameters2, boolean apiOnly, ArrayList<String> paramAPI, ArrayList<String> paramValue) {
            ...
            AvailableInfoInhMgr.this.SELCTOR.replace(0, AvailableInfoInhMgr.this.SELCTOR.length(), AvailableInfoInhMgr.GET_RELATION_VER2).append(API).append("'");
            if (AvailableInfo.DEBUG) {
                Log.i(AvailableInfoInhMgr.TAG, "start query: " + ((Object) AvailableInfoInhMgr.this.SELCTOR));
            }
            Cursor c = AvailableInfoInhMgr.this.mDB.rawQuery(AvailableInfoInhMgr.this.SELCTOR.toString(), null);
            ...
            if (count != 0 && c.moveToFirst()) {
                ...
                String[] getAPIs = c.getString(0).split(AbstractSupportedChecker.SEPARATOR);
                String[] originalAPI = c.getString(1).split(AbstractSupportedChecker.SEPARATOR);
                String[] classNames = c.getString(2).split(AbstractSupportedChecker.SEPARATOR);
                c.close();
                int originalCount = originalAPI.length;
                for (int j = 0; j < originalCount; j++) {
                    String currentOriginalAPI = originalAPI[j];
