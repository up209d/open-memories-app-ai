...
public class FocusAreaController extends AbstractController2<String> {
    ...
    public static final String[] appSupportedContrastFocusAreaValues = {MULTI, CENTER_WEIGHTED, FLEX};
    ...
    static {
        ...
        additionalFactors = new String[]{"INH_FACTOR_CAM_SET_EXC_STILL_AUTO2_MODE_TYPE_P", "INH_FACTOR_CAM_SET_EXC_STILL_AUTO2_MODE_BODY_PHASE_SHIFT_AF_TYPE_P", "INH_FACTOR_CAM_SET_EXC_STILL_AUTO2_MODE_FOCUS_DIAL_TYPE_P", "INH_FACTOR_CAM_SET_EXC_MOVIE_AUTO2_MODE_TYPE_P"};
    ...
    public Pair<Integer, Integer> getFocusPoint() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETFOCUSPOINT).append(LOG_MSG_X).append(p.first).append(", ").append(LOG_MSG_Y).append(p.second);
    ...
    public void setFocusPointIgnoreInhibit(int centerX, int centerY) {
        ...
        try {
            ...
            STRBUILD.replace(0, STRBUILD.length(), LOG_SET_FOCUS_POINT_TRY_IGNORE_INHIBIT).append(LOG_EXCEPTION);
    ...
    public void setFocusIndex(int index) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETFOCUSPOINT).append(index);
        Log.i(TAG, STRBUILD.toString());
        this.mCamSet.setParameters(emptyParam);
    }
    ...
    public int getFocusIndex() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETFOCUSPOINTINDEX).append(index);
        Log.i(TAG, STRBUILD.toString());
        return index;
    }
    ...
    public List<String> getSupportedValue() {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETSUPPORTEDFOCUSAREAMODES).append(suplist);
        Log.i(TAG, STRBUILD.toString());
    ...
    private int getSensorType(String appvalue) {
        if (MULTI.equals(appvalue) || CENTER_WEIGHTED.equals(appvalue) || FLEX.equals(appvalue) || SEMI_MULTI.equals(appvalue)) {
            return 2;
        }
    ...
    public String convertPF2App(String pf) {
        ...
        if (ret == null) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_PF).append(pf);
    ...
    public String convertApp2PF(String app) {
        ...
        if (ret == null) {
            STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_APP).append(app);
            Log.w(TAG, STRBUILD.toString());
        }
        return ret;
    }
    ...
    public String getPFValue(String tag) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_GETFOCUSAREAMODE).append(mode);
        Log.i(TAG, STRBUILD.toString());
        return mode;
    }
    ...
    public void setValueToPF(String tag, String value) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_SETFOCUSAREAMODE).append(value);
        Log.i(TAG, STRBUILD.toString());
        this.mCamSet.setParameters(emptyParam);
    }
    ...
    public boolean isUnavailableSceneFactor(String tag) {
        ...
        STRBUILD.replace(0, STRBUILD.length(), LOG_MSG_INVALID_ARGUMENT).append(LOG_MSG_TAG).append(tag);
        Log.w(TAG, STRBUILD.toString());
        return false;
    }
    ...
    public int getSensorType() {
        ...
        if (info != null) {
            ...
            return (PHASE_SHIFT_SENSOR_OTHER.equals(sensor) || PHASE_SHIFT_SENSOR_UNKNOWN.equals(sensor)) ? 2 : 1;
        }
        if (2 > CameraSetting.getPfApiVersion()) {
            return 0;
        }
        List<String> suplist = ((CameraEx.ParametersModifier) this.mCamSet.getSupportedParameters().second).getSupportedFocusAreaModes();
        if (suplist.contains(MULTI) || suplist.contains(CENTER_WEIGHTED) || suplist.contains(FLEX) || suplist.contains(SEMI_MULTI)) {
            return 2;
        }
        return 1;
    }
    ...
    private String getPhaseShiftSensorType() {
        CameraEx.LensInfo info = this.mCamSet.getLensInfo();
        if (info == null) {
            return PHASE_SHIFT_SENSOR_UNKNOWN;
        }
