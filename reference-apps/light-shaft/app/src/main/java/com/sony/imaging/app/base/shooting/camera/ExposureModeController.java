    ...
    class MovieExpModeInhibition {
        ...
        void exec(String tag) {
            if (!MovieShootingExecutor.isMovieRecording()) {
                ...
                ExposureModeController.STRBUILD.replace(0, ExposureModeController.STRBUILD.length(), ExposureModeController.MSG_MOVIE_EXP_MODE_INHIBITION).append(value);
                Log.e(ExposureModeController.TAG, ExposureModeController.STRBUILD.toString());
        ...
        void toOtherMode() {
            if (!this.p2other && true == this.other2p) {
                ...
                ExposureModeController.STRBUILD.replace(0, ExposureModeController.STRBUILD.length(), ExposureModeController.MSG_TO_OTHER).append(mode);
                Log.i(ExposureModeController.TAG, ExposureModeController.STRBUILD.toString());
                ((Camera.Parameters) p.first).setSceneMode(mode);
                this.p2other = true;
                ExposureModeController.this.mCameraSetting.setParametersDirect(p);
            }
