...
public abstract class BaseShootingExecutor {
    ...
    protected static final StringBuilder STRBUILD_CAMERA = new StringBuilder();
    protected static final StringBuilder STRBUILD_UI = new StringBuilder();
    ...
    protected static class MyBaseAutoFocusCb implements Camera.AutoFocusCallback {
        ...
        public void onAutoFocus(boolean success, Camera camera) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_ON_AUTO_FOCUS).append(BaseShootingExecutor.LOG_STATUS).append(success);
            Log.i(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    public static class MyBaseAutoFocusStartCb implements CameraEx.AutoFocusStartListener {
        ...
        public void onStart(CameraEx camex) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_ON_AUTO_FOCUS_START);
            Log.i(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    public static class MyBaseAutoFocusDoneCb implements CameraEx.AutoFocusDoneListener {
        ...
        public void onDone(int isSuccess, int[] focusarea, CameraEx camex) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_ON_AUTO_FOCUS_DONE).append(BaseShootingExecutor.LOG_STATUS).append(isSuccess);
            Log.i(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    protected static class MyBaseCustomWhiteBalanceCallback implements CameraEx.CustomWhiteBalanceCallback {
        ...
        public void onCapture(CameraEx.CustomWhiteBalanceInfo info, CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_ON_CUSTOM_WHITE_BALANCE_CAPTURE);
            Log.i(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    public static class MyBasePreviewStartCb implements CameraEx.PreviewStartListener {
        public void onStart(CameraEx cameraEx) {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_ON_PREVIEW_START);
            Log.i(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
    ...
    public static class StartPreviewRunnable implements Runnable {
        ...
        @Override // java.lang.Runnable
        public void run() {
            BaseShootingExecutor.STRBUILD_UI.replace(0, BaseShootingExecutor.STRBUILD_UI.length(), BaseShootingExecutor.LOG_UI).append(BaseShootingExecutor.LOG_START_PREVIEW).append(BaseShootingExecutor.LOG_RUNNABLE);
            Log.d(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_UI.toString());
    ...
    public void lockAutoFocus(boolean lock) {
        if (isFocusLocked != lock) {
            isFocusLocked = lock;
            if (!lock) {
                STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), LOG_CANCEL_AUTO_FOCUS).append(LOG_STATUS).append(sIsAutoFocusRequested);
                Log.d(TAG, STRBUILD_CAMERA.toString());
                if (!sIsAutoFocusRequested) {
                    STRBUILD_CAMERA.replace(0, STRBUILD_CAMERA.length(), LOG_CANCEL_AUTO_FOCUS).append(LOG_STATUS).append(LOG_LOCK);
                    Log.d(TAG, STRBUILD_CAMERA.toString());
    ...
    public void captureCustomWhiteBalance(CameraEx.CustomWhiteBalanceCallback listener) {
        ...
        STRBUILD_UI.replace(0, STRBUILD_UI.length(), LOG_UI).append(LOG_CAPTURE_CUSTOM_WHITE_BALANCE);
    ...
    protected static class CaptureCustomWhiteBalanceRunnable implements Runnable {
        ...
        @Override // java.lang.Runnable
        public void run() {
            BaseShootingExecutor.STRBUILD_CAMERA.replace(0, BaseShootingExecutor.STRBUILD_CAMERA.length(), BaseShootingExecutor.LOG_CAMERA).append(BaseShootingExecutor.LOG_CAPTURE_CUSTOM_WHITE_BALANCE);
            Log.d(BaseShootingExecutor.TAG, BaseShootingExecutor.STRBUILD_CAMERA.toString());
