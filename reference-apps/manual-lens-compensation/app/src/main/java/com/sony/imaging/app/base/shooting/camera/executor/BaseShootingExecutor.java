...
public abstract class BaseShootingExecutor {
    public static final int INTERVAL_REC_COUNT_NOT_EXECUTED = -1;
    ...
    protected static final String LOG_ON_AUTO_FOCUS_DONE_SKIP = "DONE_AUTO_FOCUS skip due to no focus change.  status:";
    ...
    static int[] last_focus_area;
    static int last_focus_status;
    ...
    protected static boolean sIsAutoReviewEnabled = true;
    ...
    public static void onDoneAutoFocus(int status, int[] area) {
        if (last_focus_status == status && ((status == 4 || status == 2) && Arrays.equals(area, last_focus_area))) {
            StringBuilder builder = getStringBuilder();
            builder.replace(0, builder.length(), LOG_CAMERA).append(LOG_ON_AUTO_FOCUS_DONE_SKIP).append(status);
            Log.i(TAG, builder.toString());
            return;
        }
        last_focus_status = status;
        if (area == null) {
            last_focus_area = null;
        } else {
            last_focus_area = (int[]) area.clone();
        }
        CameraNotificationManager.OnFocusInfo info = new CameraNotificationManager.OnFocusInfo(status, area);
        if (sAutoFocusCb != null) {
            boolean isSucceeded = status == 1 || status == 4;
    ...
    public static int getFocusStatus() {
        return last_focus_status;
    }
    ...
    public void captureCustomWhiteBalance(CameraEx.CustomWhiteBalanceCallback listener) {
        captureCustomWhiteBalance(listener, !Environment.isCustomWBOnePush());
    }

    public void captureCustomWhiteBalance(CameraEx.CustomWhiteBalanceCallback listener, boolean enableAutoReview) {
        sCustomWhiteBalanceCallback = listener;
        StringBuilder builder = getStringBuilder();
        builder.replace(0, builder.length(), LOG_UI).append(LOG_CAPTURE_CUSTOM_WHITE_BALANCE);
        Log.d(TAG, builder.toString());
        sIsAutoReviewEnabled = enableAutoReview;
        sHandlerFromMain.post(this.mCaptureCustomWhiteBalanceRunnable);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    /* loaded from: classes.dex */
    public static class CaptureCustomWhiteBalanceRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (Environment.isCustomWBOnePush()) {
                BaseShootingExecutor.sCameraEx.captureCustomWhiteBalance(BaseShootingExecutor.sMyCustomWhiteBalanceCallback, BaseShootingExecutor.sIsAutoReviewEnabled);
            } else {
