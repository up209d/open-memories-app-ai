...
public class MovieStateBase extends StateBase {
    ...
    private static final int PF_VER_CAUTION_MODE_ID_MOVIE_REC_AVAILABLE = 6;
    ...
    protected Runnable mStartRunnable = new Runnable() { // from class: com.sony.imaging.app.base.shooting.movie.MovieStateBase.1
        ...
        public void run() {
            ...
            if ((2 == ec.getRecordingMode() || 8 == ec.getRecordingMode()) && !ec.isRecordingModeChanging()) {
    ...
    public void onPause() {
        ...
        if (4 != RunStatus.getStatus()) {
            if (MovieCaptureState.FROM_STILL.equals(this.mRecFrom)) {
                ExecutorCreator.getInstance().setRecordingModeToBackup(1);
            } else if ("Interval".equals(this.mRecFrom)) {
                ExecutorCreator.getInstance().setRecordingModeToBackup(4);
            }
        }
        this.mRecFrom = null;
    }
    ...
    public Integer getCautionMode() {
        if (6 <= Environment.getVersionPfAPI()) {
            return Integer.valueOf(MovieShootingExecutor.isMovieRecording() ? CautionID.MODE_ID_MOVIE_REC_CAPTURING : CautionID.MODE_ID_MOVIE_REC);
        }
        int ret = 6;
        String format = MovieFormatController.getInstance().getValue("movie_format");
    ...
    public void startMovieRec() {
        if (Environment.hasGraphicsConstraint()) {
            ...
            getHandler().postAtFrontOfQueue(this.mStartRunnable);
        }
    }
    ...
    protected void _finishRecording() {
        ...
        if (this.data != null && (MovieCaptureState.FROM_STILL.equals(this.data.getString(MovieCaptureState.MOVIE_REC_FROM)) || CHANGE_STILL.equals(this.data.getString(CHANGE_MODE)))) {
            transitionStillMode();
        } else if (this.data != null && ("Interval".equals(this.data.getString(MovieCaptureState.MOVIE_REC_FROM)) || CHANGE_STILL.equals(this.data.getString(CHANGE_MODE)))) {
            transitionIntervaRecMode();
        } else {
    ...
    public void transitionIntervaRecMode() {
        if (this.data == null) {
            this.data = new Bundle();
        }
        HoldKeyServer.holdKey(AppRoot.USER_KEYCODE.MODE_DIAL_CHANGED, 2);
        ...
        HoldKeyServer.holdKey(AppRoot.USER_KEYCODE.SLIDE_AEL_FOCUS_CHANGED, 1);
        HoldKeyServer.holdKey(AppRoot.USER_KEYCODE.IRIS_DIAL_CHANGED, 1);
        ExecutorCreator.getInstance().setRecordingMode(4, new BackupToEE());
    }
